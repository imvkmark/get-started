---
description: 'Mac环境变量设置取决于shell类型（如bash、zsh）。系统级别存放于/etc/paths、/etc/profile等；用户级别存放在~/.bash_profile、~/.zshrc等。添加环境变量可通过export命令写入对应配置文件，或使用path_helper管理。'
lastUpdated: '2026-06-30 22:37:53'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'Mac环境变量设置取决于shell类型（如bash、zsh）。系统级别存放于/etc/paths、/etc/profile等；用户级别存放在~/.bash_profile、~/.zshrc等。添加环境变量可通过export命令写入对应配置文件，或使用path_helper管理。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/mac/setting-env-variables.html'
---
# Mac 进行环境变量设置

::: info 🔗

本文系转载增补, 原文地址(原文已失效) : [~~Mac OS X 系统的环境变量配置~~](http://yifeng.studio/2016/11/08/mac-environment-variables-configration/)
