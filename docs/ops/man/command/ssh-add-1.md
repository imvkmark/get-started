---
description: 'ssh-add命令用于将RSA或DSA私钥添加到SSH认证代理（ssh-agent），以便无需重复输入密码即可进行身份验证。'
lastUpdated: '2026-07-02 12:27:47'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'ssh-add命令用于将RSA或DSA私钥添加到SSH认证代理（ssh-agent），以便无需重复输入密码即可进行身份验证。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/man/command/ssh-add-1.html'
---
...
从代理中删除身份，而不是添加身份。如果运行了 **ssh-add** 而没有参数，则默认身份的密钥将被删除。否则，参数列表将被解释为公钥文件的路径列表，并且匹配的密钥将从代理中删除。如果在给定路径中找不到公钥，则 **ssh-add** 将附加 `*.pub*` 并重试
...
如果 **ssh-add** 需要密码，如果它是从终端运行的, 它将从当前终端读取密码短语。 如果 **ssh-add** 没有终端关联，但 DISPLAY 和 SSH_ASKPASS 已设置，则它将执行由SSH_ASKPASS指定的程序并打开X11窗口以读取密码短语。 在从 *.xsession* 或相关脚本调用 **ssh-add** 时，这特别有用。（请注意，在某些机器上，可能需要将输入从 */dev/null* 重定向以使其正常工作。）
