---
description: 'sed是一个流式文本编辑器，支持替换（s命令）、删除（d）、插入（i）、追加（a）、保持（h）、获取（G）等操作，可使用正则表达式和子串匹配（\1）、大小写转换（U/L）、定界符更改、多行编辑（e）、文件读写（r/w）等功能，还能通过脚本文件批量处理文本，如打印奇数行或匹配行的下一行。'
lastUpdated: '2026-07-01 08:32:20'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'sed是一个流式文本编辑器，支持替换（s命令）、删除（d）、插入（i）、追加（a）、保持（h）、获取（G）等操作，可使用正则表达式和子串匹配（\1）、大小写转换（U/L）、定界符更改、多行编辑（e）、文件读写（r/w）等功能，还能通过脚本文件批量处理文本，如打印奇数行或匹配行的下一行。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/man/command/sed-1.html'
---
# sed(1) - 功能强大的流式文本编辑器
