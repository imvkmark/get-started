---
description: 'localectl是用于控制系统的本地化设置与键盘布局的命令行工具。它支持查看、设置系统区域和键盘映射，并管理相关的配置文件。'
lastUpdated: '2026-06-18 09:45:05'
head:
  - - meta
    - name: 'og:title'
      content: 'localectl(1) - 控制系统的本地化与键盘布局'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'localectl是用于控制系统的本地化设置与键盘布局的命令行工具。它支持查看、设置系统区域和键盘映射，并管理相关的配置文件。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/man/command/localectl-1.html'
---
...
用于覆盖默认传递给 less 程序的命令行选项(“FRSXMK”), 如果 \$SYSTEMD_LESS 的值不含 “K” ， 并且使用 less 作为分页程序，那么 Ctrl+C 信号将会被忽略。 这将允许 less 自己处理 Ctrl+C 信号用户可能想更改两个选项
