---
description: 'ac命令从/var/log/wtmp文件读取登录和退出记录，生成以小时为单位的用户连接时间报告，并输出总计时间。该文件由init8和login1维护，若不存在则不计账。GNU ac支持-d、-y、-p等选项，可输出每日、年度或单个用户统计。需定期裁剪wtmp文件以避免过大。'
lastUpdated: '2026-06-18 08:41:56'
head:
  - - meta
    - name: 'og:title'
      content: 'ac(1) - 输出用户连接时间'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'ac命令从/var/log/wtmp文件读取登录和退出记录，生成以小时为单位的用户连接时间报告，并输出总计时间。该文件由init8和login1维护，若不存在则不计账。GNU ac支持-d、-y、-p等选项，可输出每日、年度或单个用户统计。需定期裁剪wtmp文件以避免过大。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/man/command/ac-1.html'
---
# ac(1) - 输出用户连接时间
