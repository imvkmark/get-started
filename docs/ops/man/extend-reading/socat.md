---
description: 'Socat是一款多目标中继工具，支持文件操作、网络管理、端口转发及文件传递。它可生成自签名证书用于加密连接，并能在服务端建立Shell，客户端连接后获得远程控制。此外，支持readline命令行编辑历史功能。'
lastUpdated: '2026-07-02 12:30:53'
head:
  - - meta
    - name: 'og:title'
      content: 'socat 入门教程'
  ...
  - - meta
    - name: 'og:description'
      content: 'Socat是一款多目标中继工具，支持文件操作、网络管理、端口转发及文件传递。它可生成自签名证书用于加密连接，并能在服务端建立Shell，客户端连接后获得远程控制。此外，支持readline命令行编辑历史功能。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/man/extend-reading/socat.html'
---
# socat 入门教程

::: info ℹ️

原文地址 : <a href="https://www.hi-linux.com/posts/61543.html">Socat 入门教程 - 奇妙的 Linux 世界</a>
...
```Plaintext
$ socat stdio openssl-connect:server.domain.org:4433,cert=client.pem,cafile=server.crttesttest
```
...
```Plaintext
# 连接到服务器的 7005 端口，即可获得一个 Shell
# readline 是 GNU 的命令行编辑器，具有历史功能。
