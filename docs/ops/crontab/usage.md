---
description: 'Linux中，周期执行的任务一般由cron这个守护进程来处理  ps -ef | grep croncron 读取一个或多个配置文件，这些配置文件中包含了命令行及其调用时间。cron 的配置文件称为 crontab，是 cron table的简写。一般一个用户最多只有一个crontab文件（如：root, duoli 等），其对应日志在 /var/spool/mail/root（或 /var/spool/mail/duoli）文件里说明crontab 权限问题到 /var/adm/cron/ 下一看，文件 cron.allow 和 cron.deny 是否存在用法如下：'
lastUpdated: '2025-12-19 14:28:00'
head: 
  - - meta
    - name: 'og:title'
      content: 'crontab 定时任务'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Linux中，周期执行的任务一般由cron这个守护进程来处理  ps -ef | grep croncron 读取一个或多个配置文件，这些配置文件中包含了命令行及其调用时间。cron 的配置文件称为 crontab，是 cron table的简写。一般一个用户最多只有一个crontab文件（如：root, duoli 等），其对应日志在 /var/spool/mail/root（或 /var/spool/mail/duoli）文件里说明crontab 权限问题到 /var/adm/cron/ 下一看，文件 cron.allow 和 cron.deny 是否存在用法如下：'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/crontab/usage.html'
---
# crontab 定时任务
...
-  `/var/spool/cron/`  这个目录下存放的是每个用户（包括root）的 crontab 任务，每个任务以创建者的名字命名，比如用户 tom 建的 crontab 任务对应的文件就是  `/var/spool/cron/tom` 
...
crontab 权限问题到  `/var/adm/cron/`  下一看，文件  `cron.allow`  和  `cron.deny`  是否存在用法如下：

- 如果两个文件都不存在，则只有 root 用户才能使用 crontab 命令。
- 如果  `cron.allow`  存在但  `cron.deny`  不存在，则只有列在  `cron.allow`  文件里的用户才能使用 crontab 命令，如果 root 用户也不在里面，则 root 用户也不能使用 crontab。
- 如果  `cron.allow`  不存在,  `cron.deny`  存在，则只有列在  `cron.deny`  文件里面的用户不能使用 crontab 命令，其它用户都能使用。
- 如果两个文件都存在，则列在  `cron.allow`  文件中而且没有列在  `cron.deny`  中的用户可以使用 crontab，如果两个文件中都有同一个用户，以  `cron.allow`  文件里面是否有该用户为准，如果  `cron.allow`  中有该用户，则可以使用 crontab 命令。

AIX 中 普通用户默认都有 crontab 权限，如果要限制用户使用 crontab ,就需要编辑  `/var/adm/cron/cron.deny`

HP-UNIX 中默认普通用户没得 crontab 权限 ，要想放开普通用户的 crontab 权限可以编辑  `/var/adm/cron/cron.allow`
...
CentOS 的 cron 日志默认位置在  `/var/log/cron`  中。这个文件仅记录命令的执行，而不记录结果或退出状态。默认情况下，已执行命令的输出进入用户的邮件。可以通过 crontab 内的<br />
MAILTO 变量指定电子邮件地址。

你可能应该调整  `logrotate`  规则，因为它会删除  `/var/log/secure` <br />
...
在  `crontab`  文件中，若需配置待执行的命令及其运行时间，需遵循特定格式：该文件内的每一行均包含六个字段（域）。其中，前五个字段用于指定命令的执行时间，最后一个字段则为需要执行的具体命令。
...
除了数字外，还有几个特殊符号，分别是  `_` 、 `/` 、 `-`  和  `,` 。各符号的含义如下：

-  `_` ：代表取值范围内的所有数字；
-  `/` : 表示“每”的含义，例如  `/5`  即代表“每5个单位”；
-  `-` ：代表“从某个数字到另一个数字”的范围关系；
-  `,` ：用于分隔多个离散的数字。
...
或者，执行  `crontab -e`  时指定 Shell 解释器（ `sh` ）： `SHELL=/bin/bash` （请参见上面的 crontab 编辑示例）。

若出现路径错误：例如在文件  `/var/spool/crontab/duoli`  中添加了相关命令后，在日志文件  `/var/spool/mail/duoli`  中提示无法找到  `xxx.sh`  的路径，即属于此类问题。
...
- [https://xie.infoq.cn/article/dbc768f993ecc562b31a7dd0f](https://xie.infoq.cn/article/dbc768f993ecc562b31a7dd0f)
