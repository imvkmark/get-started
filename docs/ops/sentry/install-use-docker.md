---
description: '使用Docker安装Sentry，需先安装Docker及docker-compose，配置yum源（可换阿里云），创建用户并编辑.env文件设置密钥。部署后配置nginx反向代理。常见问题包括权限问题（将用户加入docker组）、邮件配置（SMTP地址与端口）、镜像加速以及日志收集失败（需验证请求与Kafka消费）。'
lastUpdated: '2026-06-17 10:55:24'
head:
  - - meta
    - name: 'og:title'
      content: 'CentOS 使用 docker 安装 sentry'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '使用Docker安装Sentry，需先安装Docker及docker-compose，配置yum源（可换阿里云），创建用户并编辑.env文件设置密钥。部署后配置nginx反向代理。常见问题包括权限问题（将用户加入docker组）、邮件配置（SMTP地址与端口）、镜像加速以及日志收集失败（需验证请求与Kafka消费）。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/sentry/install-use-docker.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/d12d683ed5dac656fa7fc7775b677144.png'
---
# CentOS 使用 docker 安装 sentry

> 服务器配置不能低于 4U 16G 20G 磁盘
...
官网下分别有以下的两个介绍：如果你选择了通过 Docker 进行安装，其实还有更加便捷的方式 —— docker-compose 。在 github 上有一个开源项目用于部署 Sentry ，我们可以直接使用该项目进行部署，首先是克隆该项目：

地址 : https://github.com/getsentry/self-hosted
...
![](https://file.wulicode.com/feishu-images/d12d683ed5dac656fa7fc7775b677144.png)
...
# 切换目录
cd /webdata/program/self-hosted
...
![](https://file.wulicode.com/feishu-images/a107c32e6ec426109e71cf674df5067f.png)

测试下收到邮件就配置成功了

![](https://file.wulicode.com/feishu-images/87e209ec113a683d7b665418ea0af855.png)
...
![](https://file.wulicode.com/feishu-images/227d8edec8112c843ceb8089b7527c1e.png)
...
![](https://file.wulicode.com/feishu-images/72f9085e32a5f9939e3fc588f4c95852.png)

参考地址 : https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/issues/24370

### 对接 casdoor saml

在应用的管理界面, 复制 saml 元数据

![](https://file.wulicode.com/feishu-images/d1e7f7f031e45496a56efdb0a6835196.png)

填写到

![](https://file.wulicode.com/feishu-images/8cf57d55f1919559f649ed53f704286e.png)

验证并继续, 并确认信息的映射

![](https://file.wulicode.com/feishu-images/86f3aa3c2895cb539d65bb83692fa30a.png)
...
IdP User ID : Name
User Email  : Email
LastName    : DisaplayName
```

继续则授权成功

![](https://file.wulicode.com/feishu-images/cfc31c696812b77b930128d9e6cb2836.png)