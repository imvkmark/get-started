---
description: 'macOS 默认 Bash 版本较旧，升级可获取新功能与安全补丁。升级方法：通过 Homebrew 安装新版 Bash，将其加入白名单，并设为默认 Shell。注意：系统脚本可能仍依赖旧版，不建议用符号链接替换系统 Bash。'
lastUpdated: '2026-06-18 06:43:57'
head:
  - - meta
    - name: 'og:title'
      content: '在 macOS 上升级 Bash'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'macOS 默认 Bash 版本较旧，升级可获取新功能与安全补丁。升级方法：通过 Homebrew 安装新版 Bash，将其加入白名单，并设为默认 Shell。注意：系统脚本可能仍依赖旧版，不建议用符号链接替换系统 Bash。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/bash/install-at-mac.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/a1b294299d24b74658e145f026004588.png'
---
# 在 macOS 上升级 Bash

![](https://file.wulicode.com/feishu-images/a1b294299d24b74658e145f026004588.png)

::: info 🔗
原文地址 : [Upgrading Bash on macOS](https://itnext.io/upgrading-bash-on-macos-7138bd1066ba)
:::

许多 macOS 用户不知道的一件事是，他们正在使用完全过时的 Bash shell 版本。所以，强烈建议在 macOS 上使用较新版本的 Bash，因为它有很多可用的新功能。本文介绍如何执行此操作
...
如你所见，这是 [GNU Bash](https://www.gnu.org/software/bash/) **3.2** 版，其日期为 **2007年** ！此版本的 Bash 包含在 **所有** macOS 版本中，甚至是最新版。

苹果在其操作系统中包含如此旧版本的 Bash 的原因与许可有关。从 4.0 版本（3.2 的后继版本）开始，Bash 使用 GNU 通用公共许可证 v3（GPLv3），Apple 不(想) 支持。你可以 [在此处](https://www.reddit.com/r/apple/comments/7v97ls/why_doesnt_apple_use_any_gpl_v3_unix_packages_in/) 和 [此处](http://meta.ath0.com/2012/02/05/apples-great-gpl-purge/) 找到有关此 [问题](https://www.reddit.com/r/apple/comments/7v97ls/why_doesnt_apple_use_any_gpl_v3_unix_packages_in/) 的一些讨论。GNU Bash 的 3.2 版是 Apple 接受的带有 GPLv2 的最新版本，因此坚持使用这个版本。

这意味着整个世界（例如 Linux）都将使用 Bash 的新版本，而 macOS 用户则只能使用十几年前的旧版本。在撰写本文时，GNU Bash 的最新版本是 5.3.3（请参阅 [此处](https://tiswww.case.edu/php/chet/bash/bashtop.html#CurrentStatus) ），已于 2025 年发布。在本文中，我给出了将系统的默认 Shell 升级到 Bash 的最新版本的说明

## 为什么要升级？

但如果 Bash 3.2 可以正常工作，为什么还要费心升级新版呢？就我个人而言，主要原因是 **编程补全**。Bash 有个新特性是支持 特定命令的自动补全。你可能会使用自动补全功能来完成命令，文件名和变量，方法是先键入然后敲击 `Tab` \*\*以自动完成当前词句（或两次敲击 `Tab` \*\*以获取所有可能补全的列表，如果有多个以上的话） 。这是 Bash 的自动补全。
...
::: warning ⚠️
以下说明不会更改 Bash 的旧版本，而是安装新版本并将其设置为默认版本。这两个版本将在你的系统上并存，但是你可以从此处忽略旧版本
:::
...
你可以执行此操作，但是你必须避开名为 **系统完整性保护（SIP）**（[Wikipedia](https://en.wikipedia.org/wiki/System_Integrity_Protection)）的 macOS 安全功能。此功能甚至禁止 root 用户对某些目录进行写访问（这就是为什么它也称为 “rootless(root 无权限)”）的原因。这些目录在这里 列出并包括 `/bin` 。这意味着即使以 root 用户身份，你也无法执行上述命令，因为不允许你从中删除任何内容或在 `/bin` 中创建任何文件。
...
- [macos - Update bash to version 4.0 on OSX](https://apple.stackexchange.com/questions/193411/update-bash-to-version-4-0-on-osx/292760#292760)