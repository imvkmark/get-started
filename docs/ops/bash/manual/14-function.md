---
description: 'bash 函数定义语法如下：位置参数是在调用一个函数并传给它参数时创建的变量。位置参数变量表：执行 ./run.sh hello world ，然后在脚本中通过 $1 、 $2 … 读取第1 个参数、第 2 个参数。。。另外，还有几个特殊字符用来处理参数：最后，Stack Overflow 上 bash 标签下 有很多你可以学习的问题，当你遇到问题时，也是一个提问的好地方。'
...
head: 
  ...
  - - meta
    - name: 'og:description'
      content: 'bash 函数定义语法如下：位置参数是在调用一个函数并传给它参数时创建的变量。位置参数变量表：执行 ./run.sh hello world ，然后在脚本中通过 $1 、 $2 … 读取第1 个参数、第 2 个参数。。。另外，还有几个特殊字符用来处理参数：最后，Stack Overflow 上 bash 标签下 有很多你可以学习的问题，当你遇到问题时，也是一个提问的好地方。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/bash/manual/14-function.html'
---
# 14. 函数
...
<table><tbody>
  ...
  <tr>
    <td> <code>$1 &hellip; $9</code> </td>
    <td>第 1 个到第 9 个参数列表</td>
  </tr>
  <tr>
    <td> <code>${10} &hellip; ${N}</code> </td>
...
执行  `./run.sh hello world`  ，然后在脚本中通过  `$1`  、  `$2`  … 读取第1 个参数、第 2 个参数。。。
