---
description: 'Shell脚本中`if`语句根据命令返回0（真）或非0（假）执行相应分支。支持单行或换行写法、`-eq`等比较、字符串判断，以及`if-else`、`if-elif-else`和`case`多分支结构。'
lastUpdated: '2026-07-02 09:30:09'
head:
  - - meta
    - name: 'og:title'
      content: '6. 条件判断'
  ...
  - - meta
    - name: 'og:description'
      content: 'Shell脚本中`if`语句根据命令返回0（真）或非0（假）执行相应分支。支持单行或换行写法、`-eq`等比较、字符串判断，以及`if-else`、`if-elif-else`和`case`多分支结构。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/bash/refs/6-condition.html'
---
# 6. 条件判断
