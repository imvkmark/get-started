---
description: 'Shell是一种用户与操作系统交互的命令行界面，支持交互模式（手动输入命令）和非交互模式（执行脚本）。Shell脚本是由一系列Shell命令组成的文件，需指定解释器（如/bin/bash）。管道（|）用于连接命令传递数据，列表（如&&、||）实现条件执行。常见环境变量如PATH控制命令查找路径。'
lastUpdated: '2026-07-02 09:26:27'
head:
  - - meta
    - name: 'og:title'
      content: '参考'
  ...
  - - meta
    - name: 'og:description'
      content: 'Shell是一种用户与操作系统交互的命令行界面，支持交互模式（手动输入命令）和非交互模式（执行脚本）。Shell脚本是由一系列Shell命令组成的文件，需指定解释器（如/bin/bash）。管道（|）用于连接命令传递数据，列表（如&&、||）实现条件执行。常见环境变量如PATH控制命令查找路径。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/bash/refs/index.html'
---
# 参考
