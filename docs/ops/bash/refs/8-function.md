---
description: '该内容展示了命令行交互与函数调用的示例：用户选择运算方式并输入数字，计算后输出结果；同时演示了位置参数与函数参数传递，以及函数处理参数并返回运行结果（返回值为0）。'
lastUpdated: '2026-07-02 09:29:53'
head:
  - - meta
    - name: 'og:title'
      content: '8. 函数'
  ...
  - - meta
    - name: 'og:description'
      content: '该内容展示了命令行交互与函数调用的示例：用户选择运算方式并输入数字，计算后输出结果；同时演示了位置参数与函数参数传递，以及函数处理参数并返回运行结果（返回值为0）。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/bash/refs/8-function.html'
---
# 8. 函数
