---
description: 'Nginx编译选项包括指定安装路径、模块启用（如SSL、gzip、HTTP/2）、使用特定编译器优化等。常用命令如`./configure --prefix=/usr/local/nginx --with-http_ssl_module`。配置参数参考涉及worker_processes、events、http块等核心指令。'
lastUpdated: '2026-06-17 12:47:42'
head:
  - - meta
    - name: 'og:title'
      content: 'Nginx 编译选项'
  ...
  - - meta
    - name: 'og:description'
      content: 'Nginx编译选项包括指定安装路径、模块启用（如SSL、gzip、HTTP/2）、使用特定编译器优化等。常用命令如`./configure --prefix=/usr/local/nginx --with-http_ssl_module`。配置参数参考涉及worker_processes、events、http块等核心指令。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/nginx/compile-options.html'
---
# Nginx 编译选项
...
- [nginx 编译参数详解(运维不得不看) – 运维生存时间](https://www.ttlsa.com/nginx/nginx-configure-descriptions/)