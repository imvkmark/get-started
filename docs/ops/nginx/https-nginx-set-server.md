---
description: '本文介绍Nginx配置HTTPS服务器的完整流程：从使用OpenSSL生成SSL密钥和CSR文件，到配置基础HTTPS、启用HSTS策略、优化CPU运算和安全防护（防止XSS和点击劫持），并提供HTTP/HTTPS混合及基于名称的服务器配置方案。'
lastUpdated: '2026-06-18 08:15:46'
head:
  - - meta
    - name: 'og:title'
      content: 'Nginx 配置 HTTPS 服务器'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '本文介绍Nginx配置HTTPS服务器的完整流程：从使用OpenSSL生成SSL密钥和CSR文件，到配置基础HTTPS、启用HSTS策略、优化CPU运算和安全防护（防止XSS和点击劫持），并提供HTTP/HTTPS混合及基于名称的服务器配置方案。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/nginx/https-nginx-set-server.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/b7a619e7fa7402d9d31742db5efa12b1.png'
---
# Nginx 配置 HTTPS 服务器

::: info 🔗
本文是转载, 原文地址图片已失效, 原文地址: [JELLY | Nginx 配置 HTTPS 服务器 (](https://jelly.jd.com/article/6006b1045b6c6a01506c87b5)[jd.com](https://jelly.jd.com/article/6006b1045b6c6a01506c87b5)[)](https://jelly.jd.com/article/6006b1045b6c6a01506c87b5)
:::

Chrome 浏览器地址栏标志着 HTTPS 的绿色小锁头从心理层面上可以给用户专业安全的心理暗示，本文简单总结一下如何在 Nginx 配置 HTTPS 服务器，让自己站点上『绿锁』。

[https://s3-us-west-2.amazonaws.com/secure.notion-static.com/9b359ded-7e88-4802-bd05-ffb906ac6573/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/9b359ded-7e88-4802-bd05-ffb906ac6573/Untitled.png)

![](https://file.wulicode.com/feishu-images/b7a619e7fa7402d9d31742db5efa12b1.png)
...
有关 SSL 的介绍可以参阅维基百科的  [传输层安全协议](http://https//zh.wikipedia.org/wiki/%E5%82%B3%E8%BC%B8%E5%B1%A4%E5%AE%89%E5%85%A8%E5%8D%94%E8%AD%B0)  和阮一峰先生的  [《SSL/TLS协议运行机制的概述》](http://www.ruanyifeng.com/blog/2014/02/ssl_tls.html)  。

SSL 证书主要有两个功能：**加密** 和 **身份证明**，通常需要购买，也有免费的，通过第三方 SSL 证书机构颁发，常见可靠的第三方 SSL 证书颁发机构有下面几个：

![](https://file.wulicode.com/feishu-images/754af281e392153661aceee06d68f56f.png)
...
免费的证书安全认证级别一般比较低，不显示单位名称，不能证明网站的真实身份，仅起到加密传输信息的作用，适合个人网站或非电商网站。由于此类只验证域名所有权的低端 SSL 证书已经被国外各种欺诈网站滥用，因此强烈推荐部署验证单位信息并显示单位名称的 OV SSL 证书或申请最高信任级别的、显示绿色地址栏、直接在地址栏显示单位名称的 EV SSL 证书，就好像  [StarCom](http://https//www.startssl.com/)  的地址栏一样：

![](https://file.wulicode.com/feishu-images/68947d6dbbebd2702cc71b63f747a0f4.png)

更多关于购买 SSL 证书的介绍：  [SSL 证书服务，大家用哪家的？](http://https//www.zhihu.com/question/19578422)  、  [DV免费SSL证书](http://freessl.wosign.com/freessl)
...
1M 的会话缓存大概包含 4000 個会话，默认的缓存超时时间为 5 分钟，可以通过使用  [ssl_session_timeout](https://nginx.org/en/docs/http/ngx_http_ssl_module.html#ssl_session_timeout)  命令设置缓存超时时间。下面是一個拥有 10M 共享会话缓存的多核系统优化配置例子：
...
![](https://file.wulicode.com/feishu-images/1f0f51a8867fe4ba4f6873ec3ad07662.png)

浏览器在获取该响应头后，在 `max-age` 的时间内，如果遇到 HTTP 连接，就会通过 307 跳转強制使用 HTTPS 进行连接，并忽略其它的跳转设置（如 301 重定向跳转）：

![](https://file.wulicode.com/feishu-images/8bc30a10f40eb975fd7f05e88cb38e33.png)

307 跳转 **Non-Authoritative-Reason** 响应头

![](https://file.wulicode.com/feishu-images/c324d842ffd77a1dd1325733f084f63c.png)
...
针对这种情況，Google 维护了一份『HSTS 预加载列表』，列表里包含了使用了 HSTS 的站点主域名和子域名，可以通过以下页面申请加入：[https://hstspreload.appspot.com/](https://hstspreload.appspot.com/.%5D(https://hstspreload.appspot.com/))

申请的时候会先验证站点是否符合资格，一般会检验待验证的站点主域和子域是否能通过 HTTPS 连接、HTTPS 和 HTTP 配置是否有 STS Header 等信息，通过验证后，会让你确认一些限制信息，如下图：

![](https://file.wulicode.com/feishu-images/6d88f79f4c9affce1c862c151884e926.png)

当确认提交后，就会显示处理状态：

![](https://file.wulicode.com/feishu-images/c91bb94c71177f35c01e10a9a7fb0df1.png)
...
![](https://file.wulicode.com/feishu-images/11d5ea28b412f452d1eb4a808ad34fe9.png)
...
**在 0.7.14 版本之前，在独立的 server 端口中是不能选择性开启 SSL 的**。如上面的例子，SSL 只能通过使用  [ssl](https://nginx.org/en/docs/http/ngx_http_ssl_module.html#ssl)  命令为单个 server 端口开启
...
因此没有辦法设置 HTTP/HTTPS 混合服务器。于是 Nginx 新增了监听命令  [listen](https://nginx.org/en/docs/http/ngx_http_core_module.html#listen)  参数 `ssl` 來解决这个问题，**Nginx 現代版本的** `ssl` **命令并不推荐使用**
...
![](https://file.wulicode.com/feishu-images/baaaf7d3e5adeae068bca32b864a1030.png)
...
其实简单的个人博客，如果没有敏感数据交互的话，使用 http 协议通讯，一般都夠用了，页面速度还会更快，但正如文章开头所说，戴上『绿锁』，更专业更安全