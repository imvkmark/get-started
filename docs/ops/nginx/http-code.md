---
description: 'Nginx的error_page指令可自定义错误页面，例如配置为404时返回状态码404并显示404.html内容；若不加状态码，则返回200但页面仍是404.html；也可将404请求301重定向到其他域名。'
lastUpdated: '2026-06-17 12:31:13'
head:
  - - meta
    - name: 'og:title'
      content: 'Nginx 状态码配置和错误文件'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Nginx的error_page指令可自定义错误页面，例如配置为404时返回状态码404并显示404.html内容；若不加状态码，则返回200但页面仍是404.html；也可将404请求301重定向到其他域名。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//ops/nginx/http-code.html'
---
# Nginx 状态码配置和错误文件
