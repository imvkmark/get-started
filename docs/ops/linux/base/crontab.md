---
description: 'cron是Linux定时任务工具，配置文件格式为“分 时 日 月 周 命令”。支持权限控制、日志查看。常见示例：每分钟执行、每天6点、每两小时、指定星期和日期组合等。可使用run-parts执行目录下脚本，%需转义，建议用绝对路径，可输出日志。'
lastUpdated: '2026-07-02 12:24:18'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'cron是Linux定时任务工具，配置文件格式为“分 时 日 月 周 命令”。支持权限控制、日志查看。常见示例：每分钟执行、每天6点、每两小时、指定星期和日期组合等。可使用run-parts执行目录下脚本，%需转义，建议用绝对路径，可输出日志。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/linux/base/crontab.html'
---
# crontab 定时任务
...
- https://xie.infoq.cn/article/dbc768f993ecc562b31a7dd0f