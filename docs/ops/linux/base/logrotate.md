---
description: 'logrotate 是日志切割工具，可配置每周轮转、保留4周备份、创建新日志并支持压缩。配置文件位于 /etc/logrotate.d/，默认设定可被覆盖。例如，wtmp 日志记录用户登录和重启时间，通过 minsize 参数按文件大小触发轮转，而非固定周期。'
lastUpdated: '2026-07-02 12:24:46'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'logrotate 是日志切割工具，可配置每周轮转、保留4周备份、创建新日志并支持压缩。配置文件位于 /etc/logrotate.d/，默认设定可被覆盖。例如，wtmp 日志记录用户登录和重启时间，通过 minsize 参数按文件大小触发轮转，而非固定周期。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/linux/base/logrotate.html'
---
# logrotate 日志切割的使用
