---
description: '文档介绍了yum配置文件`/etc/yum.conf`与`/etc/yum.repos.d/xx.repo`的使用方法，包括检查更新、升级指定项目及列出配置。同时提供了常用Remi源的安装命令（如`yum install`和`dnf install`），并指出安装后可通过yum检查PHP与MySQL更新，但部分镜像已过期，需注意版本正确性。'
lastUpdated: '2026-07-01 13:35:17'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: '文档介绍了yum配置文件`/etc/yum.conf`与`/etc/yum.repos.d/xx.repo`的使用方法，包括检查更新、升级指定项目及列出配置。同时提供了常用Remi源的安装命令（如`yum install`和`dnf install`），并指出安装后可通过yum检查PHP与MySQL更新，但部分镜像已过期，需注意版本正确性。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/ops/linux/rhel/yum-repo-introduction.html'
---
# repo 文件说明使用和常用源
