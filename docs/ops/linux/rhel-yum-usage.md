---
description: 'yum = Yellow dog Updater, Modified主要功能是更方便的添加/删除/更新RPM包.它能自动解决包的倚赖性问题.它能便于管理大量系统的更新问题yum特点可以同时配置多个资源库(Repository)简洁的配置文件(/etc/yum.conf自动解决增加或删除rpm包时遇到的倚赖性问题使用方便保持与RPM数据库的一致性如果您不知道软件的名称，使用 search 或 provides 功能。另外，可以在任何 yum 搜索选项中使用通配符和正则表达式，来扩大搜索范围。search 功能检测所有可用的软件的名称、描述、概述和已列出'
lastUpdated: '2023-12-19 14:09:00'
head: 
  - - meta
    - name: 'og:title'
      content: 'CentOS yum的详细使用方法'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'yum = Yellow dog Updater, Modified主要功能是更方便的添加/删除/更新RPM包.它能自动解决包的倚赖性问题.它能便于管理大量系统的更新问题yum特点可以同时配置多个资源库(Repository)简洁的配置文件(/etc/yum.conf自动解决增加或删除rpm包时遇到的倚赖性问题使用方便保持与RPM数据库的一致性如果您不知道软件的名称，使用 search 或 provides 功能。另外，可以在任何 yum 搜索选项中使用通配符和正则表达式，来扩大搜索范围。search 功能检测所有可用的软件的名称、描述、概述和已列出'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/os/rhel/yum-usage.html'
---
# CentOS yum的详细使用方法
...
search 功能检测所有可用的软件的名称、描述、概述和已列出的维护者，查找匹配的值。例如，要从所有软件包中搜索与 PalmPilots<br />
相关的内容，输入：
