---
description: 'MySQL数据存储目录迁移至云盘，涉及三步骤：移动数据文件、修改配置文件、重启服务。迁移中可能出现MySQL登录失败，需检查权限与路径配置，最终完成迁移。'
lastUpdated: '2026-07-02 13:07:27'
head:
  - - meta
    - name: 'og:title'
      content: 'MySQL 数据存储目录修改'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'MySQL数据存储目录迁移至云盘，涉及三步骤：移动数据文件、修改配置文件、重启服务。迁移中可能出现MySQL登录失败，需检查权限与路径配置，最终完成迁移。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/database/mysql/extend-reading/mysql-data-directory-modify.html'
---
# MySQL 数据存储目录修改

::: info 🔗

原文地址 : <a href="https://qizhanming.com/blog/2018/05/10/how-to-change-mysql-data-directory-on-centos-7">CentOS 7 更改 MySQL 数据目录</a>
...
参考 我们需要更改  `[mysqld]`  块，更改  `datadir`  参数和  `socket`  参数 链接, 设置服务端和客户端的链接方式来解决客户端和服务端链接的问题
