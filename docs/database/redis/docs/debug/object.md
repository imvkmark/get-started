---
description: 'Redis的OBJECT命令（时间复杂度O(1）用于查看key的内部对象信息，包括引用次数、编码方式和空闲时间，便于调试和决定驱逐策略。字符串可编码为raw或int，列表可编码为ziplist或linkedlist等。'
lastUpdated: '2026-06-21 21:30:37'
head:
  - - meta
    - name: 'og:title'
      content: 'OBJECT subcommand [arguments [arguments]]'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Redis的OBJECT命令（时间复杂度O(1）用于查看key的内部对象信息，包括引用次数、编码方式和空闲时间，便于调试和决定驱逐策略。字符串可编码为raw或int，列表可编码为ziplist或linkedlist等。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/debug/object.html'
---
