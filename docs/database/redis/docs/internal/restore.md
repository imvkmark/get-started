---
description: 'RESTORE命令用于反序列化DUMP生成的键值，需指定TTL。支持REPLACE覆盖已有键，无效值会报错。示例展示了从备份恢复、重复键失败、REPLACE成功及无效数据异常。'
lastUpdated: '2026-06-21 21:32:04'
head:
  - - meta
    - name: 'og:title'
      content: 'RESTORE key ttl serialized-value [REPLACE]'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'RESTORE命令用于反序列化DUMP生成的键值，需指定TTL。支持REPLACE覆盖已有键，无效值会报错。示例展示了从备份恢复、重复键失败、REPLACE成功及无效数据异常。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/internal/restore.html'
---
