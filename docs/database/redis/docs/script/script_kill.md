---
description: 'SCRIPT KILL命令的返回值：若没有脚本在执行，返回"NOTBUSY"；成功杀死脚本时返回"OK"；尝试杀死已执行写操作的脚本时，返回"UNKILLABLE"且操作失败。'
lastUpdated: '2026-06-21 21:34:11'
head:
  - - meta
    - name: 'og:title'
      content: 'SCRIPT KILL'
  ...
  - - meta
    - name: 'og:description'
      content: 'SCRIPT KILL命令的返回值：若没有脚本在执行，返回"NOTBUSY"；成功杀死脚本时返回"OK"；尝试杀死已执行写操作的脚本时，返回"UNKILLABLE"且操作失败。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/script/script_kill.html'
---
# SCRIPT KILL
...
SCRIPT KILL 执行之后，当前正在运行的脚本会被杀死，执行这个脚本的客户端会从 [EVAL script numkeys key [key …] arg [arg …]](https://eval.md#eval) 命令的阻塞当中退出，并收到一个错误作为返回值。
