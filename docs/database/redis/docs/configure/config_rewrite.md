---
description: 'CONFIG REWRITE命令用于将当前Redis服务器的配置重写为配置文件，操作具有原子性，确保配置一致。命令执行成功返回OK，失败返回错误信息。代码示例：`redis-cli CONFIG REWRITE`。该命令可动态保存运行时配置，适用于持久化配置变更。'
lastUpdated: '2026-06-21 21:29:49'
head:
  - - meta
    - name: 'og:title'
      content: 'CONFIG REWRITE'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'CONFIG REWRITE命令用于将当前Redis服务器的配置重写为配置文件，操作具有原子性，确保配置一致。命令执行成功返回OK，失败返回错误信息。代码示例：`redis-cli CONFIG REWRITE`。该命令可动态保存运行时配置，适用于持久化配置变更。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/configure/config_rewrite.html'
---
# CONFIG REWRITE
