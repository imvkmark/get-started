---
description: 'CONFIG GET命令用于获取Redis服务器配置参数，时间复杂度O(N)。自2.0版本起可用，2.4版本部分参数无法访问，2.6版本支持所有参数。接受单个参数作为关键字，返回匹配的键-值对。例如执行CONFIG GET s，返回所有以s开头的配置参数及其值。'
lastUpdated: '2026-06-21 21:29:46'
head:
  - - meta
    - name: 'og:title'
      content: 'CONFIG GET parameter'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'CONFIG GET命令用于获取Redis服务器配置参数，时间复杂度O(N)。自2.0版本起可用，2.4版本部分参数无法访问，2.6版本支持所有参数。接受单个参数作为关键字，返回匹配的键-值对。例如执行CONFIG GET s，返回所有以s开头的配置参数及其值。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/configure/config_get.html'
---
