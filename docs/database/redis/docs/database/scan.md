---
description: 'SCAN命令用于增量迭代数据库键，支持可选的MATCH模式匹配和COUNT指定每次返回元素参考数量。它提供弱一致性保证：迭代过程中元素可能被添加或删除，但不会遗漏或重复出现。支持并发执行、中途停止，且每次迭代返回新游标，错误游标可能导致迭代不完整。返回值包含下一个游标和当前批次的键列表。'
lastUpdated: '2026-06-21 21:30:14'
head:
  - - meta
    - name: 'og:title'
      content: 'SCAN cursor [MATCH pattern] [COUNT count]'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'SCAN命令用于增量迭代数据库键，支持可选的MATCH模式匹配和COUNT指定每次返回元素参考数量。它提供弱一致性保证：迭代过程中元素可能被添加或删除，但不会遗漏或重复出现。支持并发执行、中途停止，且每次迭代返回新游标，错误游标可能导致迭代不完整。返回值包含下一个游标和当前批次的键列表。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/database/scan.html'
---
