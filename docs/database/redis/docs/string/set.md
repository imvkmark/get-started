---
description: '设置指定键的值，支持设置过期时间（秒或毫秒）以及条件（仅在键不存在或存在时设置）。返回OK表示成功，否则返回nil。'
lastUpdated: '2026-06-21 21:37:39'
head:
  - - meta
    - name: 'og:title'
      content: 'SET key value [EX seconds] [PX milliseconds] [NX|XX]'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '设置指定键的值，支持设置过期时间（秒或毫秒）以及条件（仅在键不存在或存在时设置）。返回OK表示成功，否则返回nil。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/string/set.html'
---
...
当 `SET` 命令对一个带有生存时间（TTL）的键进行设置之后，  
该键原有的 TTL 将被清除。
