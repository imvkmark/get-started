---
description: 'Redis持久化分为RDB快照和AOF日志。RDB性能高、恢复快，但可能丢失数据；AOF数据更完整、可读性强，但文件大、恢复慢。选择取决于对数据安全与性能的权衡。两者可同时使用，需合理备份与容灾。'
lastUpdated: '2026-06-21 21:38:03'
head:
  - - meta
    - name: 'og:title'
      content: '持久化（persistence）'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Redis持久化分为RDB快照和AOF日志。RDB性能高、恢复快，但可能丢失数据；AOF数据更完整、可读性强，但文件大、恢复慢。选择取决于对数据安全与性能的权衡。两者可同时使用，需合理备份与容灾。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/topic/persistence.html'
---
# 持久化（persistence）
...
请参考文章 [Redis persistence demystified](http://oldblog.antirez.com/post/redis-persistence-demystified.html) （[中文](http://blog.nosqlfan.com/html/3813.html)）。
...
（举个例子，阻塞命令 BRPOPLPUSH source destination timeout 就曾经引起过这样的 bug 。）  
测试套件里为这种情况添加了测试：  
它们会自动生成随机的、复杂的数据集，  
并通过重新载入这些数据来确保一切正常。  
...
从现在开始，  
每当 Redis 执行一个改变数据集的命令时（比如 SET key value [EX seconds] [PX milliseconds] [NX|XX]），  
...
举个例子，  
如果你对一个计数器调用了 100 次 INCR key ，  
那么仅仅是为了保存这个计数器的当前值，  
AOF 文件就需要使用 100 条记录（entry）。

然而在实际上，  
只使用一条 SET key value [EX seconds] [PX milliseconds] [NX|XX] 命令已经足以保存计数器的当前值了，  
其余 99 条记录实际上都是多余的。
