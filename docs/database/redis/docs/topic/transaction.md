---
description: 'Redis事务通过MULTI、EXEC等命令实现批量执行，但出错时不会回滚，因为设计者认为错误通常是编程问题且回滚复杂。可以使用DISCARD放弃事务。WATCH命令实现乐观锁，监控键值变化，配合EXEC实现条件执行，如实现ZPOP。Redis脚本（Lua）也支持事务性。'
lastUpdated: '2026-06-21 21:38:21'
head:
  - - meta
    - name: 'og:title'
      content: '事务（transaction）'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Redis事务通过MULTI、EXEC等命令实现批量执行，但出错时不会回滚，因为设计者认为错误通常是编程问题且回滚复杂。可以使用DISCARD放弃事务。WATCH命令实现乐观锁，监控键值变化，配合EXEC实现条件执行，如实现ZPOP。Redis脚本（Lua）也支持事务性。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/topic/transaction.html'
---
# 事务（transaction）
...
因为调用 INCR key 命令的参数格式不正确，  
所以这个 INCR key 命令入队失败。
...
回滚并不能解决编程错误带来的问题。  
举个例子，  
如果你本来想通过 INCR key 命令将键的值加上 `1` ，  
却不小心加上了 `2` ，  
又或者对错误类型的键执行了 INCR key ，  
回滚是没有办法处理这些情况的。
...
举个例子，  
假设我们需要原子性地为某个值进行增 `1` 操作（假设 INCR key 不存在）。
...
那么即使这个键过期了，  
事务仍然可以正常执行，  
关于这方面的详细情况，请看这个帖子： [http://code.google.com/p/redis/issues/detail?id=270](http://code.google.com/p/redis/issues/detail?id=270)
