---
description: '键空间通知是Redis提供的一种功能，允许客户端订阅键事件（如键过期、删除等）。它支持多种事件类型，可通过配置文件或命令启用。通知在键相关命令执行时触发，过期通知则在键被删除时发送。该功能用于实时监控数据变化。'
lastUpdated: '2026-06-21 21:38:18'
head:
  - - meta
    - name: 'og:title'
      content: '键空间通知（keyspace notification）'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '键空间通知是Redis提供的一种功能，允许客户端订阅键事件（如键过期、删除等）。它支持多种事件类型，可通过配置文件或命令启用。通知在键相关命令执行时触发，过期通知则在键被删除时发送。该功能用于实时监控数据变化。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/topic/notification.html'
---
...
比如说，对 `0` 号数据库的键 `mykey` 执行 DEL key [key …] 命令时，  
系统将分发两条消息，  
相当于执行以下两个 PUBLISH channel message 命令：
