---
description: 'Sentinel是Redis的高可用解决方案，负责监控主从节点，实现自动故障转移。其功能包括主观与客观下线检测、定期任务执行、自动发现节点、提供API和命令支持，通过发布订阅传递信息，保证故障迁移的一致性。同时处理持久化、非故障重配置、TILT模式及-BUSY状态，并有客户端实现。'
lastUpdated: '2026-06-21 21:38:28'
head:
  - - meta
    - name: 'og:title'
      content: 'sentinel'
  ...
  - - meta
    - name: 'og:description'
      content: 'Sentinel是Redis的高可用解决方案，负责监控主从节点，实现自动故障转移。其功能包括主观与客观下线检测、定期任务执行、自动发现节点、提供API和命令支持，通过发布订阅传递信息，保证故障迁移的一致性。同时处理持久化、非故障重配置、TILT模式及-BUSY状态，并有客户端实现。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/topic/sentinel.html'
---
# sentinel
...
你必须到 [Redis 项目的 Github 页面](https://github.com/antirez/redis) 克隆一份 `unstable` 分值，  
...
那么这个主服务器被标记为客观下线。
- 在一般情况下，  
每个 Sentinel 会以每 10 秒一次的频率向它已知的所有主服务器和从服务器发送 INFO [section] 命令。  
当一个主服务器被 Sentinel 标记为客观下线时，  
Sentinel 向下线主服务器的所有从服务器发送 INFO [section] 命令的频率会从 10 秒一次改为每秒一次。
...
你不可以使用 PUBLISH channel message 命令向这个服务器发送信息，  
但你可以用 SUBSCRIBE channel [channel …] 命令或者 PSUBSCRIBE pattern [pattern …] 命令，  
通过订阅给定的频道来获取相应的事件提醒。
...
- 发现主服务器已经进入客观下线状态。
- 对我们的当前纪元进行自增（详情请参考 [Raft leader election](https://www.google.com.hk/search?q=Raft+leader+election&client=ubuntu&channel=cs&oq=Raft+leader+election&aqs=chrome..69i57&sourceid=chrome&ie=UTF-8) ），  
并尝试在这个纪元中当选。
- 如果当选失败，  
那么在设定的故障迁移超时时间的两倍之后，  
重新尝试当选。  
...
- 向已下线主服务器的从服务器发送 SLAVEOF host port 命令，  
让它们去复制新的主服务器。
...
举个例子，  
当出现网络分割（[network partitions](http://en.wikipedia.org/wiki/Network_partition)）时，  
