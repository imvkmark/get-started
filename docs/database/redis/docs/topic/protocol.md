---
description: '本文介绍了网络层通信协议，涵盖新版统一请求协议及其多种回复类型（状态、错误、整数、批量、多条批量及空元素），支持多命令、流水线和内联命令，并提及高性能Redis协议分析器。'
lastUpdated: '2026-06-21 21:38:23'
head:
  - - meta
    - name: 'og:title'
      content: '通信协议（protocol）'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '本文介绍了网络层通信协议，涵盖新版统一请求协议及其多种回复类型（状态、错误、整数、批量、多条批量及空元素），支持多命令、流水线和内联命令，并提及高性能Redis协议分析器。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/topic/protocol.html'
---
# 通信协议（protocol）
...
像 LRANGE key start stop 这样的命令需要返回多个值，  
这一目标可以通过多条批量回复来完成。
...
当 SORT key [BY pattern] [LIMIT offset count] [GET pattern [GET pattern …]] [ASC | DESC] [ALPHA] [STORE destination] 命令使用 `GET pattern` 选项对一个不存在的键进行操作时，  
