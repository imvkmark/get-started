---
description: 'Redis复制功能涉及主从架构，包括关闭主持久化时的数据安全、部分重同步机制、配置只读从服务器及从服务器配置。主服务器可在至少有N个从服务器时执行写操作，确保数据一致性。'
lastUpdated: '2026-06-21 21:38:11'
head:
  - - meta
    - name: 'og:title'
      content: '复制（Replication）'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Redis复制功能涉及主从架构，包括关闭主持久化时的数据安全、部分重同步机制、配置只读从服务器及从服务器配置。主服务器可在至少有N个从服务器时执行写操作，确保数据一致性。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/topic/replication.html'
---
# 复制（Replication）
...
比如说，  
繁重的 SORT key [BY pattern] [LIMIT offset count] [GET pattern [GET pattern …]] [ASC | DESC] [ALPHA] [STORE destination] 命令可以交给附属节点去运行。
...
Redis 2.8 的这个部分重同步特性会用到一个新增的 PSYNC master_run_id offset 内部命令，  
而 Redis 2.8 以前的旧版本只有 SYNC 命令，  
不过，  
只要从服务器是 Redis 2.8 或以上的版本，  
它就会根据主服务器的版本来决定到底是使用 PSYNC master_run_id offset 还是 SYNC ：
...
另外一种方法是调用 SLAVEOF host port 命令，  
输入主服务器的 IP 和端口，  
然后同步就会开始：
...
也可以通过 CONFIG SET parameter value 命令来开启或关闭这个模式。
