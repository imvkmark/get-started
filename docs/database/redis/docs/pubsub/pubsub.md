---
description: 'PUBSUB命令用于查看Redis中发布/订阅的状态。CHANNELS列出活跃频道（去重），支持模式匹配；NUMSUB返回指定频道的订阅者数量；NUMPAT返回被订阅模式的总数（相同模式被多个客户端订阅时重复计数）。'
lastUpdated: '2026-06-22 02:44:12'
head:
  - - meta
    - name: 'og:title'
      content: 'PUBSUB subcommand [argument [argument …]]'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'PUBSUB命令用于查看Redis中发布/订阅的状态。CHANNELS列出活跃频道（去重），支持模式匹配；NUMSUB返回指定频道的订阅者数量；NUMPAT返回被订阅模式的总数（相同模式被多个客户端订阅时重复计数）。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//database/redis/docs/pubsub/pubsub.html'
---
