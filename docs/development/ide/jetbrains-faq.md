---
description: '临时解决办法:控制中心 > 菜单栏 -> 自动隐藏和显示菜单栏 更改为 永不这个东西叫做不换行空格, 也叫做硬空格但是这个字符在 IDE 中会展示为如下的样子为了保障不污染视觉我们有两种方案怎样设置在按下 Ctrl + / 时 PHPStrom 的注释风格是为 at indentation level 而非 at first column ？first lineindentation levelPreferences | Editor | Code Style | PHP  ⇒ other ⇒ Code Commenting ⇒ Line comment a'
lastUpdated: '2025-12-06 10:38:00'
head: 
  - - meta
    - name: 'og:title'
      content: 'JetBrains - FAQ'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '临时解决办法:控制中心 > 菜单栏 -> 自动隐藏和显示菜单栏 更改为 永不这个东西叫做不换行空格, 也叫做硬空格但是这个字符在 IDE 中会展示为如下的样子为了保障不污染视觉我们有两种方案怎样设置在按下 Ctrl + / 时 PHPStrom 的注释风格是为 at indentation level 而非 at first column ？first lineindentation levelPreferences | Editor | Code Style | PHP  ⇒ other ⇒ Code Commenting ⇒ Line comment a'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/development/ide/jetbrains-faq.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/notion/26/26546a94f77325fb4de655b25174d8c9.png?x-oss-process=image/resize,m_mfit,w_400'
---
# JetBrains - FAQ
...
![](https://file.wulicode.com/notion/26/26546a94f77325fb4de655b25174d8c9.png)
...
>  **不换行空格** （英语：no-break space，NBSP）是空格字符，用途是禁止自动换行。HTML页面显示时会自动合并多个连续的空白字符，但该字符是禁止合并的，因此该字符也称作“硬空格”（hard space、fixed space）。Unicode码点为：U+00A0   no-break space<br>
...
![](https://file.wulicode.com/notion/67/67f3bcd47caa5412f06a188674a6c6d7.png)

为了保障不污染视觉我们有两种方案

- 选中, 替换为空格
- 在  `Preferences | Advanced Settings`  关掉对 Unicode 字符的名称缩写

![](https://file.wulicode.com/notion/e0/e06b22da84b947a2856fa6b118667d00.png)
...
![](https://file.wulicode.com/notion/65/65215112c24c7b6b10f4388a2f330339.png)

indentation level

![](https://file.wulicode.com/notion/fe/fe931e88ea7cceb20bed9b794c1d1108.png)

`Preferences | Editor | Code Style | PHP`   ⇒  `other`  ⇒  `Code Commenting`  ⇒  `Line comment at first column`
...
![](https://file.wulicode.com/notion/bd/bd31d309a64fd8249b987b27a2c25642.png)

### 2. 使用 JDBC 链接数据库

下载 相关的 jdbc 包

![](https://file.wulicode.com/notion/54/5483f0091ce99856d7c18b171ad162df.png)

链接数据表写法  `jdbc:mysql://localhost/tablename`
...
![](https://file.wulicode.com/notion/a9/a965092e0be8492ba17fd8f112c2e7c7.png)
