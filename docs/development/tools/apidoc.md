---
description: 'apidoc 是一个 API 文档生成工具, apidoc 可以根据代码注释生成 web api 文档, apidoc 从注释生成静态 html 网页文档，不仅支持项目版本号，还支持 api 版本号, 项目约定使用此工具来生成接口文档DemoUrl : https://i.wulicode.com/docs/web/A). 系统需要安装 nodejs(略)B). 安装 apidocC). 执行生成注意: 分组名不支持中文，可修改我们的配置存放在根目录 package.json 文件中.apidoc 支持如下关键字：(下面 [ ]中括号中表示是可选写的内容，使用时'
lastUpdated: '2024-11-19 14:00:00'
head: 
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'apidoc 是一个 API 文档生成工具, apidoc 可以根据代码注释生成 web api 文档, apidoc 从注释生成静态 html 网页文档，不仅支持项目版本号，还支持 api 版本号, 项目约定使用此工具来生成接口文档DemoUrl : https://i.wulicode.com/docs/web/A). 系统需要安装 nodejs(略)B). 安装 apidocC). 执行生成注意: 分组名不支持中文，可修改我们的配置存放在根目录 package.json 文件中.apidoc 支持如下关键字：(下面 [ ]中括号中表示是可选写的内容，使用时'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/development/tools/apidoc.html'
---
# 使用 ApiDoc 编写接口文档



[apidoc](http://apidocjs.com/) 是一个 API 文档生成工具, apidoc 可以根据代码注释生成 web api 文档, apidoc 从注释生成静态 html 网页文档，不仅支持项目版本号，还支持 api 版本号, 项目约定使用此工具来生成接口文档

DemoUrl : [https://i.wulicode.com/docs/web/](https://i.wulicode.com/docs/web/)
...
apiName 的命名规范是 apiGroup + functionName; apiName 的写法规范是首字母大写的驼峰模式 例如上面的命名规范是
...
返回的类型值, 参数值, 说明必须对齐返回的参数值和真正返回的内容值必须填写完整
...
```php
/**
 * @api                 {get} /user/bank/lists [O]银行账户列表
 * @apiVersion          1.0.0
...
E). 接口中只能返回有意义的数据 没有用到的数据不需要返回, 数据返回越少,出现问题的可能性就越小

F). 列表为空也需要返回分页

G). 如果有多个返回示例可以在注释中编写多个例如:
...
这里有两个返回, 一个是 支付宝 H5 ( `ali_h5` )的返回,一个是支付宝 App( `ali_app` ) 的返回, 根据不用的类型来返回数据,如果没有默认是成功, 这里的值应该是  `success` .
