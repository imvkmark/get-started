---
description: 'Sphinx 是一个基于 Python 的文档生成项目。最早只是用来生成 Python 的项目文档，使用 reStructuredText 格式。但随着项目的逐渐完善，很多非 Python 的项目也采用 Sphinx 作为文档写作工具，甚至完全可以用 Sphinx 来写书。使用 Sphinx 生成文档的优点包括：Sphinx 依赖于 Python，并提供了 Python 包，所以使用 pip3 安装既可。这里我只安装了 sphinx 这个包。这时，通过 bash 自动补全（连续两下 tab），可以看到有几个命令，Sphinx 推荐使用 sphinx-quickstart，这是一个设'
lastUpdated: '2024-11-12 09:38:00'
head: 
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'Sphinx 是一个基于 Python 的文档生成项目。最早只是用来生成 Python 的项目文档，使用 reStructuredText 格式。但随着项目的逐渐完善，很多非 Python 的项目也采用 Sphinx 作为文档写作工具，甚至完全可以用 Sphinx 来写书。使用 Sphinx 生成文档的优点包括：Sphinx 依赖于 Python，并提供了 Python 包，所以使用 pip3 安装既可。这里我只安装了 sphinx 这个包。这时，通过 bash 自动补全（连续两下 tab），可以看到有几个命令，Sphinx 推荐使用 sphinx-quickstart，这是一个设'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/development/tools/sphinx-write-document.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/notion/94/94e9ef79fc5e60f6a089e92d2ce9cc64.png?x-oss-process=image/resize,m_mfit,w_400'
---
# 使用 Sphinx 来撰写技术文档
...
[Sphinx](http://sphinx-doc.org/) 是一个基于 Python 的文档生成项目。最早只是用来生成 [Python](https://docs.python.org/3/) 的项目文档，使用  _reStructuredText_  格式。但随着项目的逐渐完善，很多非 Python 的项目也采用 Sphinx 作为文档写作工具，甚至完全可以用 Sphinx 来写书。使用 [Sphinx 生成文档的优点](http://sphinx-doc-zh.readthedocs.org/en/latest/)包括：
...
![](https://file.wulicode.com/notion/94/94e9ef79fc5e60f6a089e92d2ce9cc64.png)
...
默认风格为  `alabaster` ，可以改成 ReadTheDocs 的风格：  `sphinx_rtd_theme` 。文件 :  _conf.py_
...
![](https://file.wulicode.com/notion/dd/dd3fc315b82acb646cceaeb86a34759d.png)
...
配置加入  `recommonmark`  解析文件 :  _conf.py_
...
![](https://file.wulicode.com/notion/29/2989ce900aebd0b8a14cf8738b25d0fa.png)
...
![](https://file.wulicode.com/notion/08/083df92558061a1fe0cb90aef76046a0.png)
...
![](https://file.wulicode.com/notion/ac/ac733d50ebc6c194a20198463f669a62.png)
...
![](https://file.wulicode.com/notion/de/deca64400a70cc7d98b566ec87bb2dbf.png)
...
![](https://file.wulicode.com/notion/6b/6b317017d0d0bbdca1ac61ecf07fcad6.png)
...
![](https://file.wulicode.com/notion/6b/6bdf49b8c64e91b01706cdca950506a8.png)

### 2) [未解决]搜索的时候无法显示所有匹配结果并跳转

![](https://file.wulicode.com/notion/d5/d5128807c5cc9b1122ab3ea703d9d7f7.png)
...
- 在域名管理中添加 DNS 的 CNAME 记录到  `readthedocs.io`  ，比如  `onebook.qiwihui.com` 
- 在项目的  `Admin`  ->  `Domains`  中设置上一步添加的域名，开启 HTTPS，保存即可。  ## 参考 简单过了一下整个文档的流程，总体来说，Sphinx非常适合用来编写项目文档，reStructuredText 比起 Markdown 也有太多的优势，值得推荐。
