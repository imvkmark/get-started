---
description: 'GitHub 的 issue 功能如同个人 TODO list，用于记录待办任务（如功能添加、bug 修复）。提交时引用 issue 编号可添加评论，使用 `fix #n` 等关键词可自动关闭对应 issue。Issue 支持 Labels（标签，如 enhancement、bug，可自定义）和 Milestone（里程碑，作为 issue 集合，与 git tag 和 release 区分）。'
lastUpdated: '2026-07-02 18:13:57'
head:
  - - meta
    - name: 'og:title'
      content: 'github issue是做什么的？'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'GitHub 的 issue 功能如同个人 TODO list，用于记录待办任务（如功能添加、bug 修复）。提交时引用 issue 编号可添加评论，使用 `fix #n` 等关键词可自动关闭对应 issue。Issue 支持 Labels（标签，如 enhancement、bug，可自定义）和 Milestone（里程碑，作为 issue 集合，与 git tag 和 release 区分）。'
  - - meta
    - name: 'og:url'
