---
description: 'JWT: Json Web Tokens JWT是一种开放标准（RFC 7519），它定义了一种紧凑且独立的方式，用于将各方之间的信息安全地传输为JSON对象。因为它是经过数字签名的，所以该信息可以进行验证并信任，无法伪造。JWT可以使用密钥进行签名，如：HMAC算法，RSA算法，ECDSA等对公钥/私钥进行签名。这是使用 JWT 的最常见方案，用户登录后，每个后续请求都将包含 JWT，允许用户访问该令牌允许的路由、服务和资源。单点登录是当今广泛使用 JWT 的一项功能，因为它的开销很小，并且能够跨不同域轻松使用。由于 JWT 可以签名（例如，使用公钥/私钥对），因此您可以'
lastUpdated: '2025-12-06 11:24:00'
head: 
  - - meta
    - name: 'og:title'
      content: 'Introduction to JSON Web Tokens / JWT 介绍'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'JWT: Json Web Tokens JWT是一种开放标准（RFC 7519），它定义了一种紧凑且独立的方式，用于将各方之间的信息安全地传输为JSON对象。因为它是经过数字签名的，所以该信息可以进行验证并信任，无法伪造。JWT可以使用密钥进行签名，如：HMAC算法，RSA算法，ECDSA等对公钥/私钥进行签名。这是使用 JWT 的最常见方案，用户登录后，每个后续请求都将包含 JWT，允许用户访问该令牌允许的路由、服务和资源。单点登录是当今广泛使用 JWT 的一项功能，因为它的开销很小，并且能够跨不同域轻松使用。由于 JWT 可以签名（例如，使用公钥/私钥对），因此您可以'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/development/common-sense/jwt-get-started.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/notion/7b/7b41fd0101327a5ae76dcceb4588b5a5.png?x-oss-process=image/resize,m_mfit,w_400'
---
# Introduction to JSON Web Tokens / JWT 介绍
...
![](https://file.wulicode.com/notion/7b/7b41fd0101327a5ae76dcceb4588b5a5.png)
...
![](https://file.wulicode.com/notion/55/5537819cb99eef29c129b0c3e0a38369.png)
...
![](https://file.wulicode.com/notion/a3/a3c75401a2b40a8c2b7741ce8218550c.png)
...
![](https://file.wulicode.com/notion/a5/a5284866a93bfa08a8c8f6662b63e9cc.png)
...
![](https://file.wulicode.com/notion/57/5703f607939589330d100009a119119d.png)

_**编码JWT和编码SAML的长度比较**_
