---
description: 'JSDOC 是一个 API 文档生成器，你只需要在代码中添加特定格式的注释，它就可以从注释中为你生成 HTML 文档全局安装：如果你更倾向项目内使用，你也可以选择：使用 JSDOC 非常简单，先为 JavaScript 文件写好注释，然后用 JSDOC 去解析即可：但这种方式只适合少量文件时使用，当文件数量多了，再加上其他参数，维护起来就会非常麻烦。 所以更好的做法是编写一份配置文件 jsdoc.json，然后通过 -c 参数来指定：无论 JSDOC 是通过全局安装，还是局部安装，都建议使用 npm scripts 来调用它，即在 package.json 的 scri'
lastUpdated: '2025-12-18 18:39:00'
head: 
  - - meta
    - name: 'og:title'
      content: '用 JSDOC 编写 JavaScript 文档'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'JSDOC 是一个 API 文档生成器，你只需要在代码中添加特定格式的注释，它就可以从注释中为你生成 HTML 文档全局安装：如果你更倾向项目内使用，你也可以选择：使用 JSDOC 非常简单，先为 JavaScript 文件写好注释，然后用 JSDOC 去解析即可：但这种方式只适合少量文件时使用，当文件数量多了，再加上其他参数，维护起来就会非常麻烦。 所以更好的做法是编写一份配置文件 jsdoc.json，然后通过 -c 参数来指定：无论 JSDOC 是通过全局安装，还是局部安装，都建议使用 npm scripts 来调用它，即在 package.json 的 scri'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/javascript/tech/jsdoc-write-js-document.html'
---
...
这样我们就可以通过  `npm run doc`  来生成文档了。如果实现修改文件后自动生成文档，只需要用类似  `nodemon`  之类的工具，监听指定文件的变化，然后再自动执行  `npm run doc`  就好了！
...
**移除 Docstrap 对 google 字体的引用** 由于引用了 google 字体，国内环境下会导致页面卡顿。打开  `node_modules\ink-docstrap\template\static\styles`  目录，将所有引用 google 字体的内容删除：

```
@import url("https://fonts.googleapis.com/css?family=Roboto:400,500");
```

指定模板：在 jsdoc 的配置文件 conf.json 下的 template 选项 配置为 docstrap/template 即可如果要手动修改模板样式：修改文件
...
- theme：皮肤模板。默认这个就挺好。Docstrap 现在提供了 13 种效果。感兴趣的，可以自己去看看其它效果：[terryweiss/docstrap](https://github.com/terryweiss/docstrap)
...
结合 JSDoc 和 Docstrap 的默认配置，我们创建一个项目使用的配置文件。在项目目录新建一个 json 文件，如： `jsdoc-conf.json` ，内容参考如下：
...
- 遇到错误或奇怪的问题时，多查阅官方文档。[JSDoc 中文文档](http://www.css88.com/doc/jsdoc/)
- 理解名称路径，有利于书写和生成更合适的文档。
- ES6 模块化方式，某些情况下对导出模块的声明，可借助   `@alias`  标签。示例
...
DocBlockr ([spadgos/sublime-jsdocs](https://github.com/spadgos/sublime-jsdocs))
