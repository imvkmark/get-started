---
description: '因为一些历史 bug, typeof null 在 JavaScript 中返回 object -- 那么我们怎么检测 null 呢?Javascript 的基础类型 null 表示为故意设置的 - 来表示指示变量已被声明但尚未分配任何值。和 null相比, 相似的值是 undefined, 没有任何对象的值,  undefined是指的一个变量已经被声明但是没有赋值任何内容, 并不是 null值.不幸的是 type of null永远返回 object, 这是一个永远不会修复的历史性的bug , 这就意味着不能够使用 typeof来验证 null值.最简单的方法来检测 '
lastUpdated: '2025-12-18 18:39:00'
head: 
  - - meta
    - name: 'og:title'
      content: '怎样在 JavaScript 中检测 null'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '因为一些历史 bug, typeof null 在 JavaScript 中返回 object -- 那么我们怎么检测 null 呢?Javascript 的基础类型 null 表示为故意设置的 - 来表示指示变量已被声明但尚未分配任何值。和 null相比, 相似的值是 undefined, 没有任何对象的值,  undefined是指的一个变量已经被声明但是没有赋值任何内容, 并不是 null值.不幸的是 type of null永远返回 object, 这是一个永远不会修复的历史性的bug , 这就意味着不能够使用 typeof来验证 null值.最简单的方法来检测 '
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/javascript/tech/how-to-check-null-in-javascript.html'
---
# 怎样在 JavaScript 中检测 null
...
不幸的是  `type of null` 永远返回  `object` , 这是一个永远不会修复的[历史性的bug](https://alexanderell.is/posts/typeof-null/) , 这就意味着不能够使用  `typeof` <br />
来验证  `null` 值.

##  `null`  是一个 false 类型的值

> “null 是一个 假值,  (i.e. 也就是当他转换成 bool 类型时候, 会变成 false)” — Josh Clanton at A Drip of JavaScript
...
当然这并不能表明  `null`  和 [其他 falsy 值](https://medium.com/coding-at-dawn/what-are-falsy-values-in-javascript-ca0faa34feb4) 的差别, 接下来我们来探究下  `==` <br />
和  `===` 在检测  `null`  值的区别

###  `==`  松散相等

> "尽管事实上 null 是一个 [假] 值, 但是在 JavaScript 中并不能将这个值和其他的 [假] 值可以进行松散的相等, 事实上, null 和 undefined<br>
...
为了确认我们确实定义了一个  `null`  值, 而不是  `undefined` 的值,<br />
...
通常来讲, 捕获  `null`  和  `undefined` 是一个比较好的方式, 都代表的是没有值. 也就是说使用  `==` 来和  `null`  进行对比是 JavaScript 中为数不多的推荐的使用方式之一,<br />
否则的话则使用  `===` 进行全等值的对比
...
> "在实际使用过程中可能会出现这样的错误 [‘null is not an object’] , 这因为你在元素加载完成之前调用这个 dom 元素导致的, 因为 DOM API<br>
> 在对象是空的情况下会返回 null" -- — Rollbar on the Top 10 JavaScript errors

如果是在加载 scirpt 之前尚未创建 dom 元素,<br />
则会发生  [TypeError](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypeError) (“ `null`  is not an object”) , 因为脚本的位置先于<br />
...
> "庆幸的是，由于它不是真正的对象，它是唯一一个 [假]值的'对象'，空的对象都是 [真]<br>
> 值" -- — Casey Morris<br>
> in Daily JS
...
ES6 函数 [Object.is()](https://medium.com/coding-at-dawn/es6-object-is-vs-in-javascript-7ce873064719) 不同于 全等于  `===`   和 松散相等  `==` 的地方是检查  `NaN` <br />
和  `-0`
