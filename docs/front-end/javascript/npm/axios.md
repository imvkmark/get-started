---
description: 'axios 是一个基于Promise 用于浏览器和 nodejs 的 HTTP 客户端或者使用cdn：可以通过将相关配置传递给 axios 来进行请求。为了方便起见，已经为所有支持的请求方法提供了别名。注意当使用别名方法时，不需要在 config 中指定 url，method 和 data 属性。帮助函数处理并发请求。您可以使用自定义配置创建axios的新实例。axios.create（[config]）可用的实例方法如下所示。 指定的配置将与实例配置合并。这些是用于发出请求的可用配置选项。 只有url是必需的。 如果未指定方法，请求将默认为GET。请求'
lastUpdated: '2025-12-18 18:41:00'
head: 
  - - meta
    - name: 'og:title'
      content: 'axios 简介'
  ...
  - - meta
    - name: 'og:description'
      content: 'axios 是一个基于Promise 用于浏览器和 nodejs 的 HTTP 客户端或者使用cdn：可以通过将相关配置传递给 axios 来进行请求。为了方便起见，已经为所有支持的请求方法提供了别名。注意当使用别名方法时，不需要在 config 中指定 url，method 和 data 属性。帮助函数处理并发请求。您可以使用自定义配置创建axios的新实例。axios.create（[config]）可用的实例方法如下所示。 指定的配置将与实例配置合并。这些是用于发出请求的可用配置选项。 只有url是必需的。 如果未指定方法，请求将默认为GET。请求'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/javascript/npm/axios.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/notion/76/76d5d083572ce14d5cdb168026034c15.png?x-oss-process=image/resize,m_mfit,w_400'
---
# axios 简介



::: info  <img src="https://file.wulicode.com/notion/7a/7ae60bcbefac2e880e8c8106a98f46a3.svg" style="width:17px;position:relative;top:4px;border:none;display:inline;">  转发文章, 原文地址<br />
...
![](https://file.wulicode.com/notion/76/76d5d083572ce14d5cdb168026034c15.png)
...
**注意** 当使用别名方法时，不需要在 config 中指定  `url` ， `method`  和  `data`  属性。
...
您可以使用自定义配置创建axios的新实例。axios.create（[config]）
