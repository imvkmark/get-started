---
description: 'npm package.json是项目的核心配置文件，包含名称、版本、描述等元数据，以及作者、许可证、入口文件、脚本、依赖管理（dependencies、devDependencies等）和引擎兼容性等关键属性。'
lastUpdated: '2026-06-21 20:26:31'
head:
  - - meta
    - name: 'og:title'
      content: 'npm package json属性详解'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'npm package.json是项目的核心配置文件，包含名称、版本、描述等元数据，以及作者、许可证、入口文件、脚本、依赖管理（dependencies、devDependencies等）和引擎兼容性等关键属性。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//front-end/javascript/npm/npm-package-json.html'
---
# npm package json属性详解

> 原文地址 : [npm package.json属性详解](https://zoucz.com/blog/2016/02/17/npm-package/)

## 概述

本文档的内容是 `package.json` 配置里边的属性含义。package.json 必须是一个严格的 json 文件，而不仅仅是 js 里边的一个对象。其中很多属性可以通过 npm-config 来生成。
...
你应该为你的模块制定一个协议，让用户知道他们有何权限来使用你的模块，以及使用该模块有哪些限制。最简单的，例如你用BSD-3-Clause 或 MIT之类的协议，如下：{ “license” : “BSD-3-Clause” }你可以在[https://spdx.org/licenses/](https://spdx.org/licenses/)这个地址查阅协议列表 。
...
也可以按如下格式缩写，npm会帮着转换:“Barney Rubble [b@rubble.com](mailto:b@rubble.com) ([http://barnyrubble.tumblr.com/)"](http://barnyrubble.tumblr.com/)%22)email和url属性实际上都是可以省略的。描述用户信息的还有一个”maintainers”（维护者）属性。
...
模块安装的时候，若是全局安装，则npm会为bin中配置的文件在bin目录下创建一个软连接（对于windows系统，默认会在C:），若是局部安装，则会在项目内的./node_modules/.bin/目录下创建一个软链接。因此，按上面的例子，当你安装myapp的时候，npm就会为cli.js在/usr/local/bin/myapp路径创建一个软链接。如果你的模块只有一个可执行文件，并且它的命令名称和模块名称一样，你可以只写一个字符串来代替上面那种配置，例如：
...
CommonJs通过directories来制定一些方法来描述模块的结构，看看npm的package.json文件

[https://registry.npmjs.org/npm/latest](https://registry.npmjs.org/npm/latest)，可以发现里边有这个字段的内容。
...
dependencies属性是一个对象，配置模块依赖的模块列表，key是模块名称，value是版本范围，版本范围是一个字符，可以被一个或多个空格分割。dependencies也可以被指定为一个git地址或者一个压缩包地址。不要把测试工具或transpilers写到dependencies中。 下面是一些写法，详见[https://docs.npmjs.com/misc/semver](https://docs.npmjs.com/misc/semver)
...
- =version1 [https://docs.npmjs.com/getting-started/using-tags](https://docs.npmjs.com/getting-started/using-tags)
...
你可以指定项目运行的node版本范围，如下：{ “engines” : { “node” : “>=0.10.3  (url)格式解析每一行的数据添加到contributors中，可以用 `#` 添加行注释
