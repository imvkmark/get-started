---
description: '在JavaScript中，`null`表示空值，属于假值。检测`null`通常使用严格相等`===`，避免`==`因类型转换误判。也可用`Object.is(null, val)`进行精确比较。`typeof null`返回`"object"`，不能可靠检测。注意`null`与`undefined`的区别，推荐使用`=== null`。'
lastUpdated: '2026-06-30 17:08:53'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: '在JavaScript中，`null`表示空值，属于假值。检测`null`通常使用严格相等`===`，避免`==`因类型转换误判。也可用`Object.is(null, val)`进行精确比较。`typeof null`返回`"object"`，不能可靠检测。注意`null`与`undefined`的区别，推荐使用`=== null`。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/front-end/javascript/extend-reading/how-to-check-null-in-javascript.html'
---
