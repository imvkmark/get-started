...
lastUpdated: '2026-07-07 10:26:45'
head:
  - - meta
    - name: 'og:title'
...
OMC 通过 Claude Code 的插件市场安装，无需手动编辑任何配置文件
...
提供 `omc` / `oh-my-claudecode` / `omc-cli` 三个 binary；`ask` / `ccg` / `team` 的 CLI 后端依赖此路径
...
| Agent | 职责 | 默认模型 |
|-|-|-|
| `qa-tester` | 测试用例编写、自动化测试 | Claude **Sonnet** |
| `code-reviewer` | PR Review、风格一致性 | Claude Sonnet |
| `security-reviewer` | 安全漏洞扫描、OWASP 检查 | Claude Sonnet |
| `critic` | 代码质量批评、潜在问题指出 | Claude Opus |
...
Skill 是 OMC 的工作流自动化单元，用一个命令触发一套完整流程。v4.14 / v4.15 新增了 `ultraqa` / `ultragoal` / `ralplan` / `autoresearch` / `sciomc` / `deep-dive` 等工作流；所有 skill 命令在 v4.15 都同时支持带命名空间形式（`/oh-my-claudecode:xxx`）和简写形式（`/xxx`，前提是别名已注册）
...
# 通过 deep-interview 的 autoresearch flag
...
> **说明**：`code-review` 与 `simplify` 不是 OMC 内置 skill，而是 Claude Code 原生命令
> 
> 如需走 OMC 流程建议用 `code-reviewer` Agent 或 `/oh-my-claudecode:code-simplifier`

#### **Setup & Maintenance（14 个）**
...
- 由 `planner` + `document-specialist` 主导
...
- 由 `ralph` 修复循环主导（原 `fixer` 已并入 `ralph`）
...
| `deepsearch` | "prompt trigger" | 代码库聚焦搜索路由，例：`deepsearch for auth middleware` |
| `ultrathink` | "prompt trigger" | 深度推理模式，例：`ultrathink about this architecture` |
...
```Bash
# Codex/Gemini 不再走 MCP server (x/g)，
...
# v4.15.0+ 新增 Grok Build worker（用于跨模型交叉 review）
/oh-my-claudecode:omc-teams 2:grok "cross-check this design decision"

# v4.15.0+ 新增 Cursor agent worker（用于实现型任务）
/oh-my-claudecode:omc-teams 1:cursor "implement the GraphQL resolver"
...
> v4.15+ 起 `omc team` 与 `/omc-teams` 已支持 6 种 worker：`claude`、`codex`、`gemini`、`agy`(antigravity)、`grok`、`cursor`
> 
> 各 worker 对应 CLI 必须已安装并通过 `omc team N:<provider> ...` 鉴权
...
OMC 的 Hooks 系统允许在 Agent 执行的关键时间点插入自定义逻辑，实现质量门控、日志记录、通知等功能。**OMC v4.15.2 提供 25 个 hooks，注册在 11 个 lifecycle event 上**（来源：仓库根目录 `hooks/hooks.json`）

**11 个 Lifecycle Events（共 25 个 hooks）：**

| # | Event | 触发时机 | OMC 注册的 hooks（timeout） |
|-|-|-|-|
| 1 | UserPromptSubmit | 用户提交 prompt | keyword-detector（10s）、skill-injector（15s） |
| 2 | SessionStart | 新会话 | session-start（5s）、project-memory-session（5s）、wiki-session-start（5s）、setup-init（init matcher，30s）、setup-maintenance（maintenance matcher，60s） |
| 3 | PreToolUse | 工具调用前 | pre-tool-enforcer（3s） |
| 4 | PermissionRequest | Bash 权限请求（OMC 增强） | permission-handler（Bash matcher，5s） |
| 5 | PostToolUse | 工具调用成功 | post-tool-verifier（3s）、project-memory-posttool（3s）、post-tool-rules-injector（3s） |
...
| 9 | PreCompact | 上下文压缩前 | pre-compact（10s）、project-memory-precompact（5s）、wiki-pre-compact（3s） |
| 10 | Stop | 主回复结束 | context-guard-stop（5s）、workflow-drift-guard（3s）、persistent-mode（10s）、code-simplifier（5s，opt-in） |
| 11 | SessionEnd | 会话结束 | session-end（30s）、wiki-session-end（30s） |
...
- **上下文管理**：notepad、project-memory、pre-compact、wiki-session-start、wiki-pre-compact、wiki-session-end
- **质量 / 验证**：permission-handler、subagent-tracker、code-simplifier（opt-in）、workflow-drift-guard
- **运维 / 平台**：session-start / -end、setup-init / -maintenance、pre/post-tool-verifier / -use-failure、post-tool-rules-injector、project-memory-session / -posttool / -precompact、context-guard-stop、verify-deliverables、pre-tool-enforcer
...
| 资源 | 地址 |
|-|-|
| 官方文档站 | https://oh-my-claudecode.dev/ |
...
| 官方 Releases | https://github.com/Yeachan-Heo/oh-my-claudecode/releases |
| Changelog | https://raw.githubusercontent.com/Yeachan-Heo/oh-my-claudecode/main/CHANGELOG.md |
