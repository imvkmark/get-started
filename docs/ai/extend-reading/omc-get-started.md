...
lastUpdated: '2026-07-06 22:33:54'
head:
  - - meta
    - name: 'og:title'
...
OMC 的 Hooks 系统允许在 Agent 执行的关键时间点插入自定义逻辑，实现质量门控、日志记录、通知等功能。**OMC v4.15.2 提供 21 个 hooks，注册在 11 个 lifecycle event 上**（来源：`docs/HOOKS.md`）。
...
**v4.14 → v4.15.2 重要变更（来源：GitHub release notes）：**

| 版本 | 关键变更 | 迁移方式 |
|-|-|-|
| v4.14.0 | **ultragoal port** 进来（OMX → OMC），`.omc/ultragoal/` 路径 | 默认无破坏 |
| v4.14.0 | skill registry 精简（plugin cache 内 native `skills/*/SKILL.md` 压缩成 shim） | 无需迁移 |
| v4.14.0 | `omc update` 修复：升级时保留已装的 `@anthropic-ai/claude-code` | 无需迁移 |
| v4.14.0 | HUD 状态栏脚本引入（若缺失，setup 会提示运行 `/oh-my-claudecode:hud setup`） | 自动提示 |
| v4.15.0 | **antigravity (agy) 新 provider**，作为 gemini 替代 | 可选安装 CLI |
| v4.15.0 | **HUD usage hint**（API key 用户可见） | 无需迁移 |
| v4.15.0 | **Magic Keywords 可配置**（#3289），通过 config.jsonc `keywords` 段 | 如需自定义才动 |
| v4.15.0 | **Claude Sonnet 5 默认支持**（#3370） | 默认即迁移 |
| v4.15.2 | **HUD 默认 preset 走 focused** 即使 settings.json 无 omcHud key（#3400） | 无需迁移 |
| v4.15.2 | **UserPromptSubmit hook timeout 调整**，keyword-detector / skill-injector 不再超时 | 无需迁移 |
| v4.15.2 | `OMC_STATE_DIR` 支持（#3397） | 可选环境变量 |
| v4.15.2 | Windows 路径处理多项修复 | 自动 |
| v4.15.2 | Discord invite 更新（#3373） | 旧链接 redirect |

**历史重要 Breaking Changes（参考）：**

| 版本 | 变更 | 迁移方式 |
|-|-|-|
| v4.1.7 | `swarm` 兼容但底层路由到 `team` | 改用 `team:` 关键词或 `/team` |
| v4.1.7 | `ultrapilot` 兼容但底层路由到 `team` | 改用 `team:` |
| v4.4.0 | Codex/Gemini MCP server（`x`/`g` provider）**移除**，改用 `/omc-teams` | 替换 `x:` / `g:` worker spec 为 `codex:` / `gemini:` 给 omc-teams |
| v4.x | `plan this` 关键词移除 | 改用 `ralplan` |
| v4.13.x → v4.14+ | `opusThreshold` / `haikuThreshold` key 移除 | 改用 tier-alias + 环境变量 |
