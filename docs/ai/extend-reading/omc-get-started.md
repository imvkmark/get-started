...
lastUpdated: '2026-07-07 09:36:04'
head:
  - - meta
    - name: 'og:title'
...
> v4.15.0+ 起 `omc team` 与 `/omc-teams` 已支持 6 种 worker：`claude`、`codex`、`gemini`、`agy`(antigravity)、`grok`、`cursor`
> 
...
Notepad 和 Project Memory 均通过 MCP 工具调用（非斜杠命令）。详见 §3.6。

```Plain Text
# Notepad（会话内，MCP 工具）
notepad_write_priority "重要约定"    # 始终加载（≤500 字符）
notepad_write_working  "当前工作状态" # 7 天自动清理
notepad_write_manual   "人工标注"    # 永不自动清理
notepad_read                        # 读取全部
notepad_stats                       # 查看状态
notepad_prune                       # 清理过期条目

# Project Memory（持久化，MCP 工具）
project_memory_write                # 整体写入（覆盖）
project_memory_add_note             # 追加 note 类条目
project_memory_add_directive        # 追加 directive（高优先级）
project_memory_read                 # 读取全部
```
