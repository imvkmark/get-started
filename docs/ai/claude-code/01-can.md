...
lastUpdated: '2026-06-17 12:19:32'
head:
  - - meta
    - name: 'og:title'
...
核心是**建立信任感,** 相信 Claude 能做的比你想象的多，然后慢慢把更多动作交出去。从安装、对话、调试，到提交代码、开 PR，这些单独看都不复杂，但串联起来会重塑你的日常开发节奏。这个阶段不需要追求技巧，最重要的一件事是：**养成开口的习惯**，遇到任何重复性的、机械性的操作，先问一句"Claude 能帮我做吗"，答案往往是肯定的

## 1.安装与配置

使用 native 方式安装, 安装方式[查看](https://code.claude.com/docs/zh-CN/overview#%E5%BC%80%E5%A7%8B%E4%BD%BF%E7%94%A8) , 由于 mac 可以统一管理, 所以推荐使用 brew, 这里不推荐使用 npm 的方式安装
...
::: tip 💡
- 不推荐使用 brew 安装, brew 版本一般会落后于官方的版本
...
- [Claude Code--火山](https://www.volcengine.com/docs/82379/1928262?lang=zh)
- [Claude Code - MiniMax](https://platform.minimaxi.com/docs/coding-plan/claude-code)
:::
...
::: info 🔗
参考资料
- [Claude Code 多模型切换实战：一个终端同时玩转 Claude、DeepSeek、Kimi](https://zhuanlan.zhihu.com/p/2041961402378834377)
:::
...
Claude Code 底层是语言模型，所有"理解"都发生在一个有限的**上下文窗口**里。目前 Claude Sonnet 4.6 支持最高 100 万 token 的上下文，听起来很大，但一个中等规模的 Spring Boot 项目轻松就有几十万行代码，全部塞进去既不现实也没必要。
