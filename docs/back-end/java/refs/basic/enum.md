---
description: '在 Java 里，Enum（枚举）属于特殊的类，其主要用途是把一组固定的常量组织起来。从 Java 5 开始，Enum 成为了 Java 语言的一部分，它让代码的可读性和安全性都得到了显著提升。下面为你详细介绍 Java 的 Enum：枚举类默认拥有一些实用的方法，这些方法是从 java.lang.Enum 继承而来的：枚举非常适合与 switch 语句配合使用，能让代码更加简洁明了。枚举常量在序列化时，仅仅是将其名称进行序列化，在反序列化时则会通过 Enum.valueOf() 方法获取对应的常量实例，这样就保证了枚举常量的唯一性。枚举是 Java 中一种强大且类型安全的'
...
head: 
  ...
  - - meta
    - name: 'og:description'
      content: '在 Java 里，Enum（枚举）属于特殊的类，其主要用途是把一组固定的常量组织起来。从 Java 5 开始，Enum 成为了 Java 语言的一部分，它让代码的可读性和安全性都得到了显著提升。下面为你详细介绍 Java 的 Enum：枚举类默认拥有一些实用的方法，这些方法是从 java.lang.Enum 继承而来的：枚举非常适合与 switch 语句配合使用，能让代码更加简洁明了。枚举常量在序列化时，仅仅是将其名称进行序列化，在反序列化时则会通过 Enum.valueOf() 方法获取对应的常量实例，这样就保证了枚举常量的唯一性。枚举是 Java 中一种强大且类型安全的'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/java/refs/basic/enum.html'
---
# Enum 解读
