---
description: '错误问题的可能性异常的层次结构RuntimeException(非检查型异常, 还包含 Error)其他异常(检查型异常)有可能抛出多个检查型异常类型如果超类方法中没有抛出任何检查型异常, 子类也不能抛出任何检查型异常包含两个构造器, 一个是默认的构造器, 一个是包含详细描述信息的构造器不管是否有异常被捕获, finally 子句中的代码都会执行继承了 AutoCloseable 和 Closeable 的接口可以堆栈轨迹(stack trace), 使用 StackWalker 来进行分析断言允许在测试期间进行一些检查, 在生产环境中自动删除这些检'
...
head: 
  ...
  - - meta
    - name: 'og:description'
      content: '错误问题的可能性异常的层次结构RuntimeException(非检查型异常, 还包含 Error)其他异常(检查型异常)有可能抛出多个检查型异常类型如果超类方法中没有抛出任何检查型异常, 子类也不能抛出任何检查型异常包含两个构造器, 一个是默认的构造器, 一个是包含详细描述信息的构造器不管是否有异常被捕获, finally 子句中的代码都会执行继承了 AutoCloseable 和 Closeable 的接口可以堆栈轨迹(stack trace), 使用 StackWalker 来进行分析断言允许在测试期间进行一些检查, 在生产环境中自动删除这些检'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/java/core/5-exception-and-log.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/notion/f5/f5c1353950bcd1ba31541ea4fa589aa2.png?x-oss-process=image/resize,m_mfit,w_400'
---
# 7. 异常, 断言, 日志
...
![](https://file.wulicode.com/notion/f5/f5c1353950bcd1ba31541ea4fa589aa2.png)
