---
description: 'Carbon 中文文档覆盖了初始化、本地化、测试、获取与设置属性（包括快捷设置）、属性检查、字符串与通用格式化、比较、修改、常量及 CarbonInterval 类等内容，还涉及 UTC 时间处理，帮助开发者高效使用这个 PHP 日期时间库。'
lastUpdated: '2026-06-29 13:30:50'
head:
  - - meta
    - name: 'og:title'
      content: 'Carbon 中文文档'
  ...
  - - meta
    - name: 'og:description'
      content: 'Carbon 中文文档覆盖了初始化、本地化、测试、获取与设置属性（包括快捷设置）、属性检查、字符串与通用格式化、比较、修改、常量及 CarbonInterval 类等内容，还涉及 UTC 时间处理，帮助开发者高效使用这个 PHP 日期时间库。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/back-end/php/packages/carbon-carbon.html'
---
# Carbon 中文文档
...
Setters 通过PHP的 \_\_set() 方法实现。值得注意的是，通过这种方式设置时间戳时，时区不会相对于时间戳而改变。如果需要改变时区的话，需要针对时区单独设置。
...
所有类似 toXXXString() 这样的方法都依赖于DateTime::format()。\_\_toString() 方法允许 Carbon 实例被打印时以一种可读性更好的方式被打印出来。
...
另外可以默认设置 \_\_toString() 方法所要显示的时间日期格式。
