---
description: '中文安装文档 :方法 1 : 官方方式方法 2 : 下载并给予权限由于默认情况下执行 composer 各种命令是去国外的 composer 官方镜像源获取需要安装的具体软件信息，所以在不使用代理、不翻墙的情况下，从国内访问国外服务器的速度相对比较慢可以使用阿里巴巴提供的 Composer 全量镜像取消当前项目配置取消全局配置列出可用的所有镜像源，前面带 * 代表当前使用的镜像使用 aliyun 镜像源可以看到 aliyun 前面有一个 * 号，代表当前使用的是 aliyun 的源自我更新 self-update将 Composer 自身升级到最'
lastUpdated: '2025-12-07 00:31:00'
head: 
  - - meta
    - name: 'og:title'
      content: 'Composer 安装和访问加速'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '中文安装文档 :方法 1 : 官方方式方法 2 : 下载并给予权限由于默认情况下执行 composer 各种命令是去国外的 composer 官方镜像源获取需要安装的具体软件信息，所以在不使用代理、不翻墙的情况下，从国内访问国外服务器的速度相对比较慢可以使用阿里巴巴提供的 Composer 全量镜像取消当前项目配置取消全局配置列出可用的所有镜像源，前面带 * 代表当前使用的镜像使用 aliyun 镜像源可以看到 aliyun 前面有一个 * 号，代表当前使用的是 aliyun 的源自我更新 self-update将 Composer 自身升级到最'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/php/composer/install-and-use-mirror.html'
---
# Composer 安装和访问加速
...
🔗 https://docs.phpcomposer.com/00-intro.html#Downloading-the-Composer-Executable
