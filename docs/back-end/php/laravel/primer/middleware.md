---
description: 'Laravel中间件过滤HTTP请求，实现认证、日志等。执行顺序按注册优先级。定义中间件含handle方法，支持前置（请求前处理）和后置（响应后处理）。注册方式：全局中间件（所有请求）、路由分配（特定路由）、中间件群组（如web、api）。中间件可接收参数，Terminable中间件在响应发送后执行额外操作。'
lastUpdated: '2026-06-29 10:42:15'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'Laravel中间件过滤HTTP请求，实现认证、日志等。执行顺序按注册优先级。定义中间件含handle方法，支持前置（请求前处理）和后置（响应后处理）。注册方式：全局中间件（所有请求）、路由分配（特定路由）、中间件群组（如web、api）。中间件可接收参数，Terminable中间件在响应发送后执行额外操作。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com/back-end/php/laravel/primer/middleware.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/91b74ac7257f17595cf95e844b17abd3.png'
---
# Laravel 中间件的介绍以及使用
...
![](https://file.wulicode.com/feishu-images/91b74ac7257f17595cf95e844b17abd3.png)
