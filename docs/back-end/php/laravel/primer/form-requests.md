---
description: 'Laravel的Form Request类将验证逻辑从控制器分离，通过创建自定义请求类实现验证简约化。支持条件判断、自定义校验规则及validateResolved方法扩展验证流程。也可在控制器中直接验证（不使用Form Request）。注意integer规则仅校验整数值，不强制变量类型。真实案例与FAQ帮助理解常见问题。'
lastUpdated: '2026-06-21 20:39:58'
head:
  - - meta
    - name: 'og:title'
      content: 'Laravel  - 表单验证类 (Form Requests) '
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: 'Laravel的Form Request类将验证逻辑从控制器分离，通过创建自定义请求类实现验证简约化。支持条件判断、自定义校验规则及validateResolved方法扩展验证流程。也可在控制器中直接验证（不使用Form Request）。注意integer规则仅校验整数值，不强制变量类型。真实案例与FAQ帮助理解常见问题。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//back-end/php/laravel/primer/form-requests.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/81d50da5ad795c0d457855c6cd6f48d3.png'
---
# Laravel  - 表单验证类 (Form Requests)
...
![](https://file.wulicode.com/feishu-images/81d50da5ad795c0d457855c6cd6f48d3.png)
...
现在控制器方法就被拦截并且随机抛出异常 (取决于请求访问的端口是奇数还是偶数, 这恐怕是有史以来最没实用价值的一个例子了, 哈哈).如你所见, 这里没有用到什么神奇的东西,  `validateResolved()`  方法是否返回 true 或者 false 并不重要. 你当然可以通过 `ValidatesWhenResolvedTrait` 这个 trait 来实现 FormRequest 中的  `failedValidation()`  的部分流程, 而在上面的例子中, 只需要抛出异常就可以了.
...
### integer 规则并非校验类型也是 integer

如下这个校验是通过的, 即使 id 的类型不是 integer, 这里要注意的是如果使用这种验证方法在 php 中再使用强类型则需要注意数据的类型转换
