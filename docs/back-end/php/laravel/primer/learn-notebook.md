---
description: '本文档是Laravel学习笔记，涵盖Composer安装、镜像设置、项目创建、权限检测、系统需求、应用密钥配置、多种运行方式（Apache/Nginx/Artisan）、事件监听、控制器、数据库配置、文件树结构、集合宏、定时任务、环境变量及路由缓存等核心内容。'
lastUpdated: '2026-06-21 20:39:36'
head:
  - - meta
    - name: 'og:title'
      content: 'Laravel 学习笔记'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '本文档是Laravel学习笔记，涵盖Composer安装、镜像设置、项目创建、权限检测、系统需求、应用密钥配置、多种运行方式（Apache/Nginx/Artisan）、事件监听、控制器、数据库配置、文件树结构、集合宏、定时任务、环境变量及路由缓存等核心内容。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//back-end/php/laravel/primer/learn-notebook.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/824f65b0f753b22138973dec208268ac.png'
---
# Laravel 学习笔记
...
![](https://file.wulicode.com/feishu-images/824f65b0f753b22138973dec208268ac.png)
...
- 设置 `key`, 这里生成的key 放在 `.env` 文件中, 用于数据的加密**生成key**
...
配置Apache我这里是 apache , 这里以 apache 为例, 这里需要配置 apache 对重写的支持 [使用 MOD_REWRITE 启用 url rewrite/url重写](http://my.oschina.net/duoli/blog/389248)
...
![](https://file.wulicode.com/feishu-images/662b6215ccc1acec77d784d3b2d3d3a2.png)
...
然后运行 `http://localhost:8000/` 也会访问到这个位置
...
- 创建视图在 `resources/views/` 目录新建文件夹 `first` 并创建文件 `index.blade.php` ;在 `index.blade.php` 文件中添加 `<h1>Hello, Laravel!</h1>` ;
...
由于数据库中定义调用了 `.env` 的方法, 所以这里使用 `.env` 的方式来加载配置之前的配置, 这里只是修改一个前缀
...
Laravel 内置的 cron 风格的令人难以置信的调度程序（与 [Indatus 的 Dispatcher](https://github.com/indatus/dispatcher)  
...
在 PHP 代码中进行性能优化并非总是我们优先考虑的问题. 但是我们对后端代码的性能优化--特别是在一些很复杂的操作上--可以对网站的请求时间产生几十甚至几百毫秒的影响.看上去不算很多, 但是[几百毫秒的差异对于你的应用的用户感知速度来说可能意味着巨大的差异](http://www.nytimes.com/2012/03/01/technology/impatient-web-users-flee-slow-loading-sites.html?pagewanted=all) ( 链接可能需要扶墙访问).

一个只有几百条路由定义的 Laravel 站点, 框架光注册路由就需要半秒多的时间. 不过以后不用担心这个问题了, 因为 Laravel 的**路由缓存(route caching)**, 可以大大优化路由的性能(闭包方式定义的路由不能缓存, 所以该把所有的闭包路由定义都移到控制器中了).

### 使用路由缓存

老实说, 使用路由缓存很简单. 有一个新的 Artisan 命令 `route:cache` 可以把 `routes.php` 文件中的所有路由定义序列化

会对所有的路由进行一次解析然后把解析结果缓存起来. 如果你曾经用过 Handlebars 模板的预编译的话, Laravel 的路由缓存机制跟它有点类似.
