---
description: '本内容介绍了Laravel Collective HTML组件的安装与使用，涵盖表单CSRF保护、模型绑定、各类输入元素（文本框、密码、隐藏域、多选、单选、数字、日期、文件选择器）、下拉列表（含默认值、占位符、分组、范围、月份）、按钮、自定义宏以及URL链接生成（link_to、link_to_asset、link_to_route、link_to_action）。'
lastUpdated: '2026-06-17 22:45:02'
head:
  - - meta
    - name: 'og:title'
      content: 'Forms & HTML 组件 (laravelcollective-html) '
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
...
首先通过 composer 来安装这个包, 编辑项目的 `composer.json` 文件. 在 `require` 部分 加入 `laravelcollective/html` :
...
> Note: HTML 表单仅仅支持 POST 和 GET方法, PUT 和DELETE 方法将会使用一个隐藏域_method 添加到 form 表单中来欺骗实现
...
这个可以让你快速的使用模型数据来创建表单, 也能轻松的在服务器校验错误之后重新发布表单. > **Note:** 使用 `Form::model`方法的时候一定要使用 `Form::close`来关闭表单!
