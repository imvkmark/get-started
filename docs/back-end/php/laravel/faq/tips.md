...
> I believe this is designed this way to limit number of SQL queries. If you need to get actual data saved in the database, you need to obtain this record explicitely, just the way you did :) INSERT query doesn’t return actual row.
...
```PHP
// 传值 bad
route('area.establish', ['parent_id' => $item['areaid']])
// 不传值 good
route('area.establish', [$item->areaid]])
```

这两个哪个写起来更简洁呢?

因为使用 route 的时候接收到的参数在控制器传参数进行获取`public function establish($parent_id)` 这种方法才能够接收到数据的.

> 使用 `$request->input('parent_id')` 根本获取不到东西

### 使用对象和对象的错误提示

```PHP
# 使用对象的好处
route('area.create', [$item['areaid']])   # 如果不存在字段, 则报 undefined index 错误
route('area.create', [$item->area_id])    # 这里不报错的.

# 使用映射过的对象的好处是容易识记
route('area.create', [$item->areaid])    # 这里不报错的.
route('area.create', [$item->area_id])   # 使用映射过的字段更便于记忆, 减少浏览器的 `typo` 错误
```

### Form 使用 post 方法提交可以不填写 ‘method’
...
因为这里的更新就是批量的, 并且使用的方式不是更新一个, SQL 使用 case when 减少数据库查询
...
### 队列中运行延迟时候需要确保存在 `delay()` 方法

使用 `Illuminate\Bus\Queueable` 这个 Trait,`.env` 中的 `QUEUE_DRIVER` 能够是 sync

### `empty()` 和 `Collection::empty()` 方法不同
...
![](https://file.wulicode.com/feishu-images/28d5715fa52e2d257a62ee6fea7c6e39.png)
...
用户输入的数据有存在被跨站攻击的可能, 所以使用以下来进行 html 实体转换可以保障数据安全
