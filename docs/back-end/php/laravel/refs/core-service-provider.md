---
description: '该内容围绕Laravel Service Provider展开，涵盖其定义（作为引导器和组织者）、安装与注册包、使用`--dev`安装、在`register()`中注册自身、管理服务容器、实现接口并注入、切换类、`Register()`与`Boot()`的区别、延迟提供者（`$defer`和`provides()`）以及重新启动等关键步骤。'
lastUpdated: '2026-06-17 22:43:11'
head:
  - - meta
    - name: 'og:title'
      content: '深入探讨 Service Provider'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '该内容围绕Laravel Service Provider展开，涵盖其定义（作为引导器和组织者）、安装与注册包、使用`--dev`安装、在`register()`中注册自身、管理服务容器、实现接口并注入、切换类、`Register()`与`Boot()`的区别、延迟提供者（`$defer`和`provides()`）以及重新启动等关键步骤。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//back-end/php/laravel/refs/core-service-provider.html'
  - - meta
    - name: 'og:image'
      content: 'https://file.wulicode.com/feishu-images/913697070bbe5aba1e344c22b8e104d2.png'
---
# 深入探讨 Service Provider

::: info 🔗
文章系转载, 原文地址(原文已失效) : [~~深入探討 Service Provider~~](http://oomusou.io/laravel/laravel-service-provider/)
:::
...
初学者第一次接触 service provider，应该是在安装 package 时，以安装`Laravel Debugbar`为例，一开始我们会使用 composer 安装 :详细请参考[如何使用 Laravel Debugbar?](http://oomusou.io/laravel/laravel-debugbar/)
...
以 Laravel Debugbar 为例，使用 composer 安装完 package 之后，只是将 package 安装在`/vendor/barryvdh/laravel-debugbar` 目录下，此时 Laravel 还不知道有这个 package，必须在`config/app.php`中`注册`该 package 所提供的 service provider，Laravel 才知道 Laravel Debugbar 的存在，并在Laravel 启动时载入时透过 Laravel Debugbar 的 service provider 去载入 Laravel Debugbar。
...
由于 Laravel Debugbar 不适合在`正式上线`主机使用，因此我们特别判断`application enviromnent`是否为`local`，若为 local，才使用`$this->app->register()`  
...
也就是说，原本`config/app.php`是直接载入 Laravel Debugbar 提供的 service provider，现在改成载入`自己写`的 service provider，加入了判断 application  
environment，再自行载入 Laravel Debugbar 提供的 service provider，以避免在正式上线主机载入 Laravel Debugbar。

![](https://file.wulicode.com/feishu-images/913697070bbe5aba1e344c22b8e104d2.png)
...
搭配 controller，不过当初并没有垫 interface，现在我们加上了`PostControllerInterface`，并使用 service provider 管理。
...
，因为我们刚刚将`PostRepositoryServiceProvider`改成`deferred provider` ，所以必须删除`service.json`重新建立
