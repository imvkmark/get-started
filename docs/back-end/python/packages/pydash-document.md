---
description: 'Pydash中文文档涵盖了数组、链式操作、集合、函数、数值、对象、谓词、字符串及公共工具等多个模块的实用函数，提供了丰富的数据处理与操作接口。'
lastUpdated: '2026-06-17 19:11:34'
head:
  - - meta
    - name: 'og:title'
      content: 'Pydash 中文文档'
  ...
  - - meta
    - name: 'og:description'
      content: 'Pydash中文文档涵盖了数组、链式操作、集合、函数、数值、对象、谓词、字符串及公共工具等多个模块的实用函数，提供了丰富的数据处理与操作接口。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//back-end/python/packages/pydash-document.html'
---
# Pydash 中文文档
...
与zip\_（）相反，该方法将元素组拆分为列表，列表中的每个元素组在其对应的索引处组成
...
此方法与reduce\_（）类似，只是它从右到左迭代集合的元素。
...
此函数类似于reduce\_（），只是它返回了reduce操作中每个中间值的列表。
...
与filter\_（）相反，此方法返回谓词不返回truthy的集合元素。
...
检查谓词是否返回集合中任何元素的真值。谓词由三个参数调用：（value、index|key、collection）。如果为谓词传递了属性名，则创建的map\_（）样式谓词将返回给定元素的属性值。如果为谓词传递对象，则创建的matches（）样式谓词将为具有给定对象属性的元素返回True，否则返回False。
...
此方法与set\_（）类似，只是它接受被调用以生成路径对象的自定义程序。如果自定义程序返回未定义的路径，则由该方法处理创建。自定义程序由三个参数调用：（nested_value、key、nested_object）。
...
此方法与set\_（）类似，只是接受updater来生成要设置的值。使用update_with（）自定义路径创建。使用一个参数调用更新程序：（value）。
...
与property\_（）类似，只是它返回路径中每个路径的值列表。
...
property\_（）的倒数。此方法创建一个函数，该函数返回obj上给定键的键值。
