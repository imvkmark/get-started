---
description: '`if __name__ == ''__main__''` 用于判断当前模块是否作为脚本直接运行，`__name__` 在直接运行时为 `''__main__''`。`__main__.py` 文件定义包的入口，配合 `python -m` 可将包作为脚本执行。'
lastUpdated: '2026-06-21 19:56:56'
head:
  - - meta
    - name: 'og:title'
      content: 'Python 中的 if __name__ == ''__main__'' 该如何理解'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '`if __name__ == ''__main__''` 用于判断当前模块是否作为脚本直接运行，`__name__` 在直接运行时为 `''__main__''`。`__main__.py` 文件定义包的入口，配合 `python -m` 可将包作为脚本执行。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//back-end/python/extend-reading/name-main.html'
---
# Python 中的 if \_\_name\_\_ == '\_\_main\_\_' 该如何理解

::: info 🔗
原文地址 [~~https://blog.konghy.cn/2017/04/24/python-entry-program/~~](https://blog.konghy.cn/2017/04/24/python-entry-program/)
:::
...
这才是我们想要的效果。`if __name__ == '__main__'` 就相当于是 Python **模拟的程序入口**。Python 本身并没有规定这么写，这只是一种编码习惯。由于模块之间相互引用，不同模块可能都有这样的定义，而入口程序只能有一个。到底哪个入口程序被选中，这取决于 `__name__` 的值。
...
这里的 tornado 和 tornado.web 就被称为模块的模块名。如果一个模块被直接运行，则其没有包结构，其 `__name__` 值为 `__main__`。例如在上例中，我们直接运行 c.py 文件（python a/b/c.py），输出结果如下：

```Plaintext
__main__
```

所以，`if __name__ == '__main__'` 我们简单的理解就是： **如果模块是被直接运行的，则代码块被运行，如果模块是被导入的，则代码块不被运行**。实际上，这个问题还可以衍生出其他的一些知识点，例如 `__main__.py` 文件与 Python 的 `-m` 参数。

## \*\*`__main**.py__` 文件与 python -m
...
3、 **main.py 文件是一个包或者目录的入口程序**

不管是用 `python package` 还是用 `python -m package` 运行时，`__main.py__` 文件总是被执行。
