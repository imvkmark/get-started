---
description: '在CentOS上通过pyenv安装Python时，可能因缺少`_ctypes`模块报错。需先安装编译依赖，如`libffi-devel`，再使用pyenv安装最新版Python。参考FAQ解决模块缺失问题。'
lastUpdated: '2026-06-21 20:39:17'
head:
  - - meta
    - name: 'og:title'
      content: 'CentOS 安装 Python'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '在CentOS上通过pyenv安装Python时，可能因缺少`_ctypes`模块报错。需先安装编译依赖，如`libffi-devel`，再使用pyenv安装最新版Python。参考FAQ解决模块缺失问题。'
  - - meta
    - name: 'og:url'
      content: 'https://www.wulicode.com//back-end/python/primer/install-at-centos.html'
---
# CentOS 安装 Python
