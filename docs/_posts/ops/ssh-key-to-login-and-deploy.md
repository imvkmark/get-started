...
密钥锁码在使用私钥时必须输入，这样就可以保护私钥不被盗用。当然，也可以留空，实现无密码登录。<br />现在，在指定的目录中生成了一个有两个密钥文件。id_rsa 为私钥，id_rsa.pub 为公钥。
...
所谓部署， 我的理解就是在用户保证代码质量的前提下, 将代码能够快速的自动部署到目标服务器上的一种手段.<br />具体步骤参照 [配置 SSH 公钥](https://help.coding.net/docs/project-settings/ssh.html)
> 主机配置同上
...
来查看问题出现的原因, 根据出现的问题来进行处理<br />我这里出现的问题是在 `pledge: network` 之后出现卡顿<br />处理结果
