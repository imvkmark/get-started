...
```
像上边展示出来的结果似的, `null` 仅仅和他自身还有 `undefined`可以松散相等, 但是和其他的 [假] 值不等.<br />这对于检查值的缺失很有用 --  `null` 和 `undefined`两者都表示没有值, 因此他们可以 松散相等(他们拥有相同的值, 但是类型不同).<br />所以我们写代码的时候如果要验证一个值是否存在值可以使用 `== null` 来检测他是 `null` 还是 `undefined`
