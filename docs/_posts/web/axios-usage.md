...
原文地址: [axios全攻略](https://ykloveyxk.github.io/2017/02/25/axios%E5%85%A8%E6%94%BB%E7%95%A5/)<br />axios 是一个基于Promise 用于浏览器和 nodejs 的 HTTP 客户端
...
**注意**<br />当使用别名方法时，不需要在 config 中指定 `url`，`method` 和 `data` 属性。
...
### 创建实例
您可以使用自定义配置创建axios的新实例。<br />axios.create（[config]）
```
